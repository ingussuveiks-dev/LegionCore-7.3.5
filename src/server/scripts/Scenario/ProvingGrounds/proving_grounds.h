...
{
    ScenarioChallenge(uint32 currentWave, uint32 maxWaves, ChallengeMedals::MedalType medalType, uint32 timerDuration)
        : WaveCurrent(currentWave), WaveMax(maxWaves), Medal(medalType), TimerDuration(timerDuration) { }

    uint32 WaveCurrent;
    uint32 WaveMax;
    ChallengeMedals::MedalType Medal;
    uint32 TimerDuration;
};

inline void SendProvingGroundsScenarioState(Player* player, uint32 /*mapId*/, uint32 step, uint32 /*progress*/,
    uint32 /*criteriaTreeId*/ = 0, uint32 scenarioId = 237, uint32 /*bonusObjectiveId*/ = 0,
    ScenarioChallenge const* challenge = nullptr)
{
    if (!player)
        return;

    WorldPackets::Scenario::ScenarioState packet;
    packet.ScenarioID = scenarioId;
    packet.DifficultyID = scenarioId == 248 ? 12 : 1;
    packet.CurrentStep = scenarioId == 248 ? (step ? 599 : 598) : 550;
    packet.ActiveSteps.push_back(packet.CurrentStep);

    if (challenge)
    {
        packet.WaveCurrent = challenge->WaveCurrent;
        packet.WaveMax = challenge->WaveMax;
        packet.TimerDuration = challenge->TimerDuration;
    }
