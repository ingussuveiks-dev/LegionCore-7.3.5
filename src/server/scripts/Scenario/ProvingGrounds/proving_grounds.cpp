...
namespace
{
    ...
    Player* GetTrialOwner(Creature* creature)
    {
        if (!creature || !creature->GetInstanceScript())
            return nullptr;

        return ObjectAccessor::GetPlayer(*creature, creature->GetInstanceScript()->GetGuidData(PLAYER_DATA));
    }

    void SendChallengeStartTimer(Creature* creature, uint32 currentDuration)
    {
        if (Player* player = GetTrialOwner(creature))
        {
            WorldPackets::WorldState::StartElapsedTimer packet;
            packet.Timer.TimerID = WORLD_TIMER_TYPE_CHALLENGE_MODE;
            packet.Timer.CurrentDuration = currentDuration;
            player->SendDirectMessage(packet.Write());
        }
    }

    void SendChallengeStopElapsedTimer(Creature* creature)
    {
        if (Player* player = GetTrialOwner(creature))
        {
            WorldPackets::Misc::StopElapsedTimer packet;
            packet.TimerID = WORLD_TIMER_TYPE_CHALLENGE_MODE;
            player->SendDirectMessage(packet.Write());
        }
    }
}
...
protected:
    ...
    AreaTrigger* GetAreaTrigger() const { return _areaTrigger; }
...
public:
    ...
    {
        explicit AI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { handler.Initialize(areaTrigger); }
    ...
    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override { return new AI(areaTrigger); }
};

struct customCreatureAI : public ScriptedAI
{
    explicit customCreatureAI(Creature* creature) : ScriptedAI(creature), summons(me) { }
            ...
            if (Creature* ourMember = ObjectAccessor::GetCreature(*me, interruptMemberGUID))
            {
                ...
                switch (ourMember->GetEntry())
                {
                    case NPC_KAVAN_THE_ARCANIST:
                        ourMember->AI()->Talk(TALK_KAVAN_INTERRUPT, me->GetGUID());
                        break;
                    case NPC_KI_THE_ASSASSIN:
                        ourMember->AI()->Talk(TALK_KI_INTERRUPT, me->GetGUID());
                        break;
                }
...
struct npc_proving_grounds_illusionary_guardian : public provingGroundsAI
{
    ...
    void EnterCombat(Unit* who) override
    {
        ...
        {
            PrepareChanneledCast(me, frand(0.0f, 2 * M_PI), SPELL_DEFEND);
...
struct npc_proving_grounds_illusionary_banana_tosser : public provingGroundsAI
{
    ...
    void EnterCombat(Unit* who) override
    {
            ...
            .Schedule(Seconds(2), [this](TaskContext context)
        {
            ...
            hasReached = true;
            context.Repeat(Milliseconds(me->movespline->Duration()));
        });
    }
    ...
    void EnterCombat(Unit* who) override
    {
            ...
            .Schedule(Seconds(5), [this](TaskContext context)
        {
            if (Unit* target = me->getVictim())
            {
                ...
                PrepareChanneledCast(me, me->GetAngle(target), SPELL_WING_BLAST);

                scheduler
                    .Schedule(Seconds(1), [this](TaskContext context)
                {
                    RemoveChanneledCast(me, targetGUID);
                });
            }
...
struct npc_proving_grounds_illusionary_conqueror : public provingGroundsAI
{
    ...
    void EnterCombat(Unit* who) override
    {
            ...
            .Schedule(Seconds(5), [this](TaskContext context)
        {
            ...
            {
                targetGUID = target->GetGUID();
                PrepareChanneledCast(me, me->GetAngle(target), SPELL_POWERFUL_SLAM);
...
struct npc_proving_grounds_kavan_the_arcanist : public customCreatureAI
{
    ...
    {
        if (HealthBelowPct(90) && attacker && attacker->GetTargetGUID() == me->GetGUID())
        {
    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            for (auto&& itr : invAllowTrialType)
                if (player->HasAchieved(itr.first))
        ...
        switch (actionId)
        {
            case ACTION_DD_BRONZE_TRIAL:
                    ...
                    .Schedule(Milliseconds(10 * IN_MILLISECONDS), [this](TaskContext context)
                {
                    ...
                    if (me->GetInstanceScript())
                        SendChallengeStopElapsedTimer(me);
                    ...
                    if (Player* trialOwner = ObjectAccessor::GetPlayer(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(PLAYER_DATA) : ObjectGuid::Empty))
                    {
                        ...
                        SendProvingGroundsScenarioState(trialOwner, 1148, 1, 0, 33614, 248, 0, &waveData);
                    }
            ...
            case ACTION_FAIL_TRIAL:
                ...
                // Repeat Scenario
                if (Player* trialOwner = ObjectAccessor::GetPlayer(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(PLAYER_DATA) : ObjectGuid::Empty))
                    SendProvingGroundsScenarioState(trialOwner, 1148, DATA_SELECT_A_TRIAL, 0);
...
class npc_proving_grounds_trial_master_rotun_queue : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            if (!player->GetGroup() && !sLFGMgr->GetQueueId(player->GetGUID()))
                player->ADD_GOSSIP_ITEM_DB(player->GetDefaultGossipMenuForSource(creature), 0, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
