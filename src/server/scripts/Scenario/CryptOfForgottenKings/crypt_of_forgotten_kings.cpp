                    ...
                    switch (eventId)
                    {
                        case EVENT_BREATH_OF_HATE:
                            if (Unit* vict = me->getVictim())
                            {
                                ...
                                PrepareChanneledCast(me, me->GetAngle(vict), SPELL_BREATH_OF_HATE);
                            }
    ...
    public:
        ...
        struct npc_cfk_crypt_guardian_vaultAI : public ScriptedAI
        {
            ...
            void MovementInform(uint32 type, uint32 pointId) override
            {
                ...
                if (me->GetEntry() == NPC_CRYPT_GUARDIAN && pointId == 2 && me->GetPositionX() < 796.0f)
                    nonCombatEvents.ScheduleEvent(EVENT_REACH_POS, 100);
            }
    ...
    public:
        ...
        struct npc_cfk_abomination_of_angerAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                targetGUID = ObjectGuid::Empty;
                if (instance)
                    instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_FANCY_FOORWORK), 1);
            }
...
class spell_cfk_shadow_step : public SpellScriptLoader
{
    public:
        ...
        class spell_cfk_shadow_step_SpellScript : public SpellScript
        {
            ...
            void Register() override
            {
                OnEffectHitTarget += SpellEffectFn(spell_cfk_shadow_step_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_TELEPORT_UNITS_FACE_CASTER);
            }
        };
...
class spell_cfk_trigger_lightning_trap : public SpellScript
{
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        if (Creature* caster = GetCaster()->ToCreature())
        {
            if (Unit* target = GetHitUnit())
            {
                caster->AI()->Talk(TALK_INTRO, target->GetGUID());
        ...
        if (Creature* caster = GetCaster()->ToCreature())
            if (GetHitPlayer())
                if (InstanceScript* instance = caster->GetInstanceScript())
                    instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_FANCY_FOORWORK), 0);
    }
                ...
                if (Creature* Abomination = ObjectAccessor::GetCreature(*player, instance->GetGuidData(NPC_ABOMINATION_OF_ANGER)))
                {
                    if (uint32 abomInit = Abomination->AI()->GetData(TYPE_ABOMINATION_INIT))
                    {
                        if (abomInit < 2)
                        {
                            ...
                            Abomination->AI()->Talk(TALK_INTRO, player->GetGUID());

                            if (Scenario* scenario = sScenarioMgr->GetScenario(player->GetInstanceId()))
                                scenario->SetCurrentStep(DATA_ABOMINATION_OF_ANGER + 1);
                        }
                    }
                }
            }
...
void AddSC_crypt_of_forgotten_kings()
{
    ...
    RegisterSpellScript(spell_cfk_trigger_lightning_trap);
    RegisterSpellScript(spell_cfk_anger_eff);
    RegisterSpellScript(spell_achiev_fight_with_anger);
    RegisterAuraScript(spell_cfk_hate_periodic);
