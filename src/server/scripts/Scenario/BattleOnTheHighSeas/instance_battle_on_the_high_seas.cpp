    ...
    public:
        ...
        struct instance_battle_on_the_high_seas_InstanceMapScript : public InstanceScript
        {
            ...
            void Initialize() override
            {
                ...
                DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_KEEP_THOSE_BOMBS_AWAY), 1);
            }
            ...
            void OnPlayerEnter(Player* player) override
            {
                ...
                uint8 step = DATA_BOARDING_PARTY;
                if (chapterOne >= DONE)
                    step = DATA_EXPLOSIVES_ACQUICITION;
                if (chapterTwo >= DONE)
                    step = DATA_SMITHEREENS;
                if (chapterThree >= DONE)
                    step = DATA_DEFEAT_THE_ADMIRAL;
            ...
            void SetData(uint32 type, uint32 data) override
            {
                switch (type)
                {
                    case DATA_BOARDING_PARTY:
                        chapterOne = data;
                        SetBossState(DATA_BOARDING_PARTY, EncounterState(data));

                        if (data == DONE)
                        {
                            SendScenarioCriteria(GetData(FACTION_DATA) ? 32775 : 30722);
                            SetScenarioStep(DATA_EXPLOSIVES_ACQUICITION);
                        }
                    ...
                    case DATA_EXPLOSIVES_ACQUICITION:
                        chapterTwo = data;
                        SetBossState(DATA_EXPLOSIVES_ACQUICITION, EncounterState(data));
                        ...
                        {
                            if (data == DONE)
                                SetScenarioStep(DATA_SMITHEREENS);
                    ...
                    case DATA_SMITHEREENS:
                        chapterThree = data;
                        SetBossState(DATA_SMITHEREENS, EncounterState(data));
                        ...
                        else
                        {
                            if (data == DONE)
                            {
                                SendScenarioCriteria(GetData(FACTION_DATA) ? 32780 : 30724);
                                SetScenarioStep(DATA_DEFEAT_THE_ADMIRAL);
                            }
                    ...
                    {
                        chapterFour = data;
                        SetBossState(DATA_DEFEAT_THE_ADMIRAL, EncounterState(data));
                        SendScenarioCriteria(GetData(FACTION_DATA) ? 32782 : 30734);
                    ...
                    case EXPLOSIVES_COUNT_DATA:
                        ...
                        SendScenarioCriteria(GetData(FACTION_DATA) ? 32777 : 32658, explosiveBarrelsCount);
                    ...
                    case PLANT_COUNT_DATA:
                        ++plantBarrelCount;
                        SendScenarioCriteria(GetData(FACTION_DATA) ? 32779 : 30732, plantBarrelCount);
