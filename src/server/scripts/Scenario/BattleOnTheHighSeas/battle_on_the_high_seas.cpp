...
// equivalent behaviour local to this scenario instead of changing the core.
enum HighSeasTargetPriority
{
    PRIORITY_VICTIM,
    PRIORITY_NOT_VICTIM,
    PRIORITY_CHANNELED,
...
struct HighSeasCreatureAI : public ScriptedAI
{
    HighSeasCreatureAI(Creature* creature) : ScriptedAI(creature), summons(me) { }
    ...
    void ExecuteTargetEvent(uint32 spellId, uint32 repeat, uint32 eventId, uint32 currentEventId,
        HighSeasTargetPriority priority = PRIORITY_VICTIM)
    {
...
struct npc_high_seas_cannoneer : public HighSeasCreatureAI
{
    npc_high_seas_cannoneer(Creature* creature) : HighSeasCreatureAI(creature) { }
...
struct npc_high_seas_swashbuckler : public HighSeasCreatureAI
{
    npc_high_seas_swashbuckler(Creature* creature) : HighSeasCreatureAI(creature) { }
...
// Horde Captain 71445
struct npc_high_seas_horde_captain : public HighSeasCreatureAI
{
    npc_high_seas_horde_captain(Creature* creature) : HighSeasCreatureAI(creature)
    {
        triggered = false;
        delay = 0;
        calculationCount = 13;
...
struct npc_high_seas_horde_squalsharper : public HighSeasCreatureAI
{
    npc_high_seas_horde_squalsharper(Creature* creature) : HighSeasCreatureAI(creature)
    {
...
struct npc_high_seas_triad_of_lieutenants : public HighSeasCreatureAI
{
    npc_high_seas_triad_of_lieutenants(Creature* creature) : HighSeasCreatureAI(creature) { }
...
struct npc_high_seas_explosive_barrel_trigger : public ScriptedAI
{
    ...
    void Reset() override
    {
        me->SetDisplayId(sObjectMgr->GetCreatureDisplay(me->GetCreatureTemplate()->Modelid[1]));
    }
...
struct npc_high_seas_admiral_hodgson : public HighSeasCreatureAI
{
    npc_high_seas_admiral_hodgson(Creature* creature) : HighSeasCreatureAI(creature) { }
...
class spell_high_seas_cannon_launch : public SpellScript
{
    ...
    void HandleHit(SpellEffIndex effIndex)
    {
        ...
        {
            caster->AddDelayedEvent(500, [caster]()
            {
...
class spell_high_seas_sticky_bomb : public SpellScript
{
    ...
    void HandleHit(SpellEffIndex effIndex)
    {
        if (Creature* caster = GetCaster()->ToCreature())
            if (Unit* target = GetHitUnit())
                caster->AI()->Talk(TALK_SPECIAL_1, target->GetGUID());
    }
    ...
    void HandleHit(SpellEffIndex effIndex)
    {
        if (Player* target = GetHitPlayer())
            if (InstanceScript* instance = target->GetInstanceScript())
                instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_KEEP_THOSE_BOMBS_AWAY), 0);
    }
...
class sat_high_seas_fire : public AreaTriggerScript
{
public:
    sat_high_seas_fire() : AreaTriggerScript("sat_high_seas_fire") { }

    struct sat_high_seas_fireAI : AreaTriggerAI
    {
        sat_high_seas_fireAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }

        void OnUnitEnter(Unit* unit) override
        {
            if (Player* player = unit->ToPlayer())
                if (player->IsAlive())
                    player->CastSpell(player, SPELL_FIRE_EFF, true);
        }

        void OnUnitExit(Unit* unit) override
        {
            if (Player* player = unit->ToPlayer())
                player->RemoveAurasDueToSpell(SPELL_FIRE_EFF);
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new sat_high_seas_fireAI(areaTrigger);
    }
};

void AddSC_battle_on_the_high_seas()
{
    RegisterCreatureAI(npc_high_seas_cannoneer);
    RegisterCreatureAI(npc_high_seas_swashbuckler);
    RegisterCreatureAI(npc_high_seas_cannon_exploder);
    RegisterCreatureAI(npc_high_seas_admiral_hagman);
    RegisterCreatureAI(npc_high_seas_transport_cannon);
    RegisterCreatureAI(npc_high_seas_horde_captain);
    RegisterCreatureAI(npc_high_seas_horde_squalsharper);
    RegisterCreatureAI(npc_high_seas_zip);
    RegisterCreatureAI(npc_high_seas_zip_back);
    RegisterCreatureAI(npc_high_seas_bomb);
    RegisterCreatureAI(npc_high_seas_fuse);
    RegisterCreatureAI(npc_high_seas_triad_of_lieutenants);
    RegisterCreatureAI(npc_high_seas_explosive_barrel_trigger);
    RegisterCreatureAI(npc_high_seas_explosive_barrel);
    RegisterCreatureAI(npc_high_seas_admiral_hodgson);
    RegisterCreatureAI(npc_high_seas_net_climber);

    RegisterSpellScript(spell_high_seas_cannon_launch);
    RegisterSpellScript(spell_high_seas_activate_random_barrel_selector);
    RegisterAuraScript(spell_high_seas_bomb);
    RegisterSpellScript(spell_high_seas_bomb_eff);
    RegisterSpellScript(spell_high_seas_activate_random_barrel_eff);
    RegisterSpellScript(spell_high_seas_sticky_bomb);
    RegisterAuraScript(spell_high_seas_sticky_bomb_aura);
    RegisterSpellScript(spell_high_seas_barrel_explosion_eff);
    RegisterSpellScript(spell_high_seas_shotgun_targeting);
    RegisterSpellScript(spell_high_seas_shotgun);
    RegisterAuraScript(spell_high_seas_shotgun_ammo);
    RegisterSpellScript(spell_high_seas_gather_explosives);
    RegisterSpellScript(spell_high_seas_priming);
    RegisterSpellScript(spell_high_seas_bombs_away_eff);
    new sat_high_seas_fire();
}
