...
// The Pandaria source used a project-specific combat helper. Keep its behavior
// local to this scenario instead of changing LegionCore's shared AI API.
enum BloodSnowTargetPriority
{
    PRIORITY_VICTIM,
    PRIORITY_NOT_VICTIM,
    PRIORITY_CHANNELED,
...
struct BloodSnowCreatureAI : public ScriptedAI
{
    BloodSnowCreatureAI(Creature* creature) : ScriptedAI(creature), summons(me) { }
    ...
    void ExecuteTargetEvent(uint32 spellId, uint32 repeat, uint32 eventId, uint32 currentEventId,
        BloodSnowTargetPriority priority = PRIORITY_VICTIM)
    {
...
struct npc_bloodsnow_scout_stonebeard : public ScriptedAI
{
    ...
    void DoAction(int32 actionId) override
    {
        if (!hasTriggered && actionId == ACTION_START_INTRO)
        {
            ...
            me->ExitVehicle();
            me->GetMotionMaster()->MoveJump(stonebeardJumpPos, 20.0f, 15.0f);
...
struct npc_bloodsnow_frostmane_flesh_eater : public BloodSnowCreatureAI
{
    npc_bloodsnow_frostmane_flesh_eater(Creature* creature) : BloodSnowCreatureAI(creature)
    {
...
struct npc_bloodsnow_frostmane_headhunter : public BloodSnowCreatureAI
{
    npc_bloodsnow_frostmane_headhunter(Creature* creature) : BloodSnowCreatureAI(creature)
    {
        scoutGUID = ObjectGuid::Empty;
    }
...
struct npc_bloodsnow_frostmane_snowstalker : public BloodSnowCreatureAI
{
    npc_bloodsnow_frostmane_snowstalker(Creature* creature) : BloodSnowCreatureAI(creature)
    {
        scoutGUID = ObjectGuid::Empty;
    }
...
struct npc_bloodsnow_frostmane_berserker : public BloodSnowCreatureAI
{
    npc_bloodsnow_frostmane_berserker(Creature* creature) : BloodSnowCreatureAI(creature)
    {
        scoutGUID = ObjectGuid::Empty;
    }
...
struct npc_bloodsnow_frostmane_bonechiller : public BloodSnowCreatureAI
{
    npc_bloodsnow_frostmane_bonechiller(Creature* creature) : BloodSnowCreatureAI(creature)
    {
        scoutGUID = ObjectGuid::Empty;
    }
...
// Farastu 70474
struct npc_bloodsnow_farastu : public BloodSnowCreatureAI
{
    npc_bloodsnow_farastu(Creature* creature) : BloodSnowCreatureAI(creature) { }
...
// Freeze Solid 70987
struct npc_bloodsnow_freeze_solid : public BloodSnowCreatureAI
{
    npc_bloodsnow_freeze_solid(Creature* creature) : BloodSnowCreatureAI(creature) { }
...
struct npc_bloodsnow_hekima_the_wise : public BloodSnowCreatureAI
{
    npc_bloodsnow_hekima_the_wise(Creature* creature) : BloodSnowCreatureAI(creature) { }
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        if (me->GetInstanceScript())
        {
            me->GetInstanceScript()->SetData(DATA_VICTORY_OF_DEATH, DONE);
        }
    }
...
struct npc_bloodsnow_frostmane_witch_doctor : public BloodSnowCreatureAI
{
    npc_bloodsnow_frostmane_witch_doctor(Creature* creature) : BloodSnowCreatureAI(creature)
    {
        scoutGUID = ObjectGuid::Empty;
    }
...
struct npc_bloodsnow_ironforge_guardian : public BloodSnowCreatureAI
{
    npc_bloodsnow_ironforge_guardian(Creature* creature) : BloodSnowCreatureAI(creature) { }
...
struct npc_bloodsnow_shimmerweed_basket : public BloodSnowCreatureAI
{
    npc_bloodsnow_shimmerweed_basket(Creature* creature) : BloodSnowCreatureAI(creature) { }
...
struct npc_bloodsnow_scout_boldbrew : public BloodSnowCreatureAI
{
    npc_bloodsnow_scout_boldbrew(Creature* creature) : BloodSnowCreatureAI(creature)
    {
        prisonersCount = 0;
        delay = 0;
        hasTriggered = false;
...
struct npc_bloodsnow_scout_forgefellow : public BloodSnowCreatureAI
{
    npc_bloodsnow_scout_forgefellow(Creature* creature) : BloodSnowCreatureAI(creature)
    {
        prisonersCount = 0;
        delay = 0;
        hasTriggered = false;
...
struct npc_bloodsnow_zandalari_rage_banner : public BloodSnowCreatureAI
{
    npc_bloodsnow_zandalari_rage_banner(Creature* creature) : BloodSnowCreatureAI(creature)
    {
        scoutGUID = ObjectGuid::Empty;
    }
    ...
    void HandleRemove(AuraEffect const* pAuraEffect, AuraEffectHandleModes eMode)
    {
        if (Unit* owner = GetOwner()->ToUnit())
        {
            ...
            if (GetCaster() && GetCaster()->ToCreature())
                GetCaster()->ToCreature()->AI()->Talk(TALK_INTRO, owner->GetGUID());
        }
    }
        ...
        if (Creature* caster = GetCaster()->ToCreature())
            if (caster->GetEntry() == NPC_HEKIMA_THE_WISE)
                if (InstanceScript* instance = caster->GetInstanceScript())
                    instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_HEKIMAS_HEAL_HALTER), 0);
    }
...
class sat_bloodsnow_shimmerweed : public AreaTriggerScript
{
public:
    sat_bloodsnow_shimmerweed() : AreaTriggerScript("sat_bloodsnow_shimmerweed") { }

    struct sat_bloodsnow_shimmerweedAI : AreaTriggerAI
    {
        sat_bloodsnow_shimmerweedAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger), triggered(false) { }
        ...
        void OnUnitEnter(Unit* unit) override
        {
            ...
            triggered = true;
            player->CastSpell(player, SPELL_SHIMMERWEED_EFF, true);

            if (Unit* caster = at->GetCaster())
                caster->RemoveAurasDueToSpell(SPELL_SHIMMERWEED_VISUAL);

            if (InstanceScript* instance = player->GetInstanceScript())
                instance->SetData(DATA_SHIMMERWEED, 1);
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new sat_bloodsnow_shimmerweedAI(areaTrigger);
    }
};
...
{
    RegisterCreatureAI(npc_bloodsnow_varian_wrynn);
    RegisterCreatureAI(npc_bloodsnow_mountainer_grimbolt);
    RegisterCreatureAI(npc_bloodsnow_roasting_spit);
    RegisterCreatureAI(npc_bloodsnow_scout_stonebeard);
    RegisterCreatureAI(npc_bloodsnow_frostmane_flesh_eater);
    RegisterCreatureAI(npc_bloodsnow_frostmane_headhunter);
    RegisterCreatureAI(npc_bloodsnow_frostmane_snowstalker);
    RegisterCreatureAI(npc_bloodsnow_frostmane_berserker);
    RegisterCreatureAI(npc_bloodsnow_frostmane_bonechiller);
    RegisterCreatureAI(npc_bloodsnow_farastu);
    RegisterCreatureAI(npc_bloodsnow_freeze_solid);
    RegisterCreatureAI(npc_bloodsnow_hekima_the_wise);
    RegisterCreatureAI(npc_bloodsnow_frostmane_witch_doctor);
    RegisterCreatureAI(npc_bloodsnow_ironforge_guardian);
    RegisterCreatureAI(npc_bloodsnow_shimmerweed_basket);
    RegisterCreatureAI(npc_bloodsnow_scout_boldbrew);
    RegisterCreatureAI(npc_bloodsnow_scout_forgefellow);
    RegisterCreatureAI(npc_bloodsnow_zandalari_rage_banner);

    RegisterSpellScript(spell_bloodsnow_free_scout_stonebeard);
    RegisterAuraScript(spell_bloodsnow_freeze_solid_launcher);
    RegisterAuraScript(spell_bloodsnow_freeze_solid_eff);
    RegisterSpellScript(spell_bloodsnow_zandalari_rage_eff);
    RegisterSpellScript(spell_bloodsnow_hekimas_wisdom);
    new sat_bloodsnow_shimmerweed();
