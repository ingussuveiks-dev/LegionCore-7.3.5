...
namespace
{
    void PrepareDaggerChanneledCast(Creature* creature, float orientation, uint32 spellId)
    {
        creature->AttackStop();
    ...
    void RemoveDaggerChanneledCast(Creature* creature, ObjectGuid targetGuid)
    {
        creature->SetReactState(REACT_AGGRESSIVE);
        if (Unit* target = ObjectAccessor::GetUnit(*creature, targetGuid))
            creature->AI()->AttackStart(target);
        else if (Player* target = creature->SelectNearestPlayer(100.0f))
            creature->AI()->AttackStart(target);
    }
}
...
// The 5.4.8 source used a project-specific combat helper that is not part of
// LegionCore. Keep the scenario behaviour local instead of changing the core.
enum DaggerTargetPriority
{
    PRIORITY_VICTIM,
    PRIORITY_NOT_VICTIM,
    PRIORITY_CHANNELED,
...
{
    customCreatureAI(Creature* creature) : ScriptedAI(creature), summons(me) { }
    ...
    void ExecuteTargetEvent(uint32 spellId, uint32 repeat, uint32 eventId, uint32 currentEventId,
        DaggerTargetPriority priority = PRIORITY_VICTIM)
    {
...
void SendDaggerScenarioCriteria(InstanceScript* instance, uint32 treeId)
{
    if (!instance)
        return;

    Scenario* scenario = sScenarioMgr->GetScenario(instance->instance->GetInstanceId());
                    ...
                    case ACTION_START_INTRO:
                            ...
                            .Schedule(Milliseconds(delay += 1000), [this](TaskContext context)
                        {
                            ...
                            if (Creature* rakgor = ObjectAccessor::GetCreature(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(NPC_RAKGOR_BLOODRAZOR) : ObjectGuid::Empty))
                            {
                                ...
                                rakgor->DespawnOrUnsummon(rakgor->movespline->Duration());
                            }
                        });
                            ...
                            .Schedule(Milliseconds(delay += 2000), [this](TaskContext context)
                        {
                            ...
                            if (Creature* bodyguard = ObjectAccessor::GetCreature(*me, getAnyKorkronBodyguardGUID()))
                            {
                                ...
                                bodyguard->DespawnOrUnsummon(bodyguard->movespline->Duration());
                            }
                        });
    ...
    void DoAction(int32 actionId) override
    {
        switch (actionId)
        {
            ...
            case ACTION_MOVE_TO_BOAT:
                ...
                uint32 delay = me->movespline->Duration();
                scheduler
                    .Schedule(Milliseconds(delay), [this](TaskContext context)
                {
                    Talk(TALK_SPECIAL_1);
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/) override
    {
        ...
        if (HealthBelowPct(20) && !reachLowHealth)
        {
            ...
            if (InstanceScript* instance = me->GetInstanceScript())
                instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_NO_EGG_LEFT_BEHIND), 0);
        }
    }
    ...
    void DoAction(int32 actionId) override
    {
        switch (actionId)
        {
            ...
            case ACTION_IN_CAVE:
            {
                ...
                scheduler
                    .Schedule(Milliseconds(me->movespline->Duration()), 1, [this](TaskContext context)
                {
                    if (me->GetInstanceScript())
        ...
        switch (actionId)
        {
            case ACTION_DEFEAT_BROODMASTER:
            {
                    ...
                    .Schedule(Milliseconds(delay += 3000), [this](TaskContext context)
                {
                    ...
                    if (Scenario* scenario = sScenarioMgr->GetScenario(me->GetMap()->GetInstanceId()))
                        scenario->SetCurrentStep(DATA_INVESTIGATION);
            ...
            case ACTION_GONNA_KILL_VOLJIN:
                if (Creature* voljin = ObjectAccessor::GetCreature(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(NPC_VOLJIN) : ObjectGuid::Empty))
                {
                    ...
                    uint32 delay = me->movespline->Duration() + 3000;
                    scheduler
                        .Schedule(Milliseconds(delay), [this](TaskContext context)
                    {
                        Talk(TALK_SPECIAL_5);
...
class spell_dagger_in_dark_shadowstep_eff : public SpellScript
{
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_dagger_in_dark_shadowstep_eff::HandleEffectHit, EFFECT_0, SPELL_EFFECT_5);
    }
};
    ...
    void HandleAfterCast()
    {
        if (Unit* caster = GetCaster())
        {
            ...
            if (Creature* moguTablet = caster->FindNearestCreature(tabletEntry, 100.0f, true))
            {
                ...
                // Update Criteria
                uint32 criteriaTreeId = GetSpellInfo()->Id == SPELL_INVESTIGATION ? CRITERIA_TREE_WEST_STATUE : CRITERIA_TREE_EAST_STATUE;

                if (caster->GetInstanceScript())
                {
                    SendDaggerScenarioCriteria(caster->GetInstanceScript(), criteriaTreeId);
...
class sat_dagger_in_dark_mogu_rune_powerup : public AreaTriggerScript
{
public:
    sat_dagger_in_dark_mogu_rune_powerup() : AreaTriggerScript("sat_dagger_in_dark_mogu_rune_powerup") { }

    struct sat_dagger_in_dark_mogu_rune_powerupAI : AreaTriggerAI
    {
        sat_dagger_in_dark_mogu_rune_powerupAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger), triggered(false) { }
        ...
        void OnUnitEnter(Unit* unit) override
        {
            ...
            if (Player* player = unit->ToPlayer())
            {
                ...
                triggered = true;
                player->CastSpell(player, SPELL_MOGU_POWER, true);

                if (Creature* caster = at->GetCaster() ? at->GetCaster()->ToCreature() : nullptr)
                {
                    caster->AI()->Talk(TALK_INTRO, player->GetGUID());
                    caster->DespawnOrUnsummon(1 * IN_MILLISECONDS);
                }
            }
        }
    };
    ...
    {
        return new sat_dagger_in_dark_mogu_rune_powerupAI(areaTrigger);
    }
};

// 573. Summoned by 133892 - Big Bad Voodoo
class sat_dagger_in_dark_big_bad_voodoo : public AreaTriggerScript
{
public:
    sat_dagger_in_dark_big_bad_voodoo() : AreaTriggerScript("sat_dagger_in_dark_big_bad_voodoo") { }

    struct sat_dagger_in_dark_big_bad_voodooAI : AreaTriggerAI
    {
        sat_dagger_in_dark_big_bad_voodooAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }

        void OnUnitEnter(Unit* unit) override
        {
            if (Player* player = unit->ToPlayer())
                if (player->IsAlive())
                    player->CastSpell(player, SPELL_BIG_BAD_VOODOO_EFF, true);
        }

        void OnUnitExit(Unit* unit) override
        {
            if (Player* player = unit->ToPlayer())
                player->RemoveAurasDueToSpell(SPELL_BIG_BAD_VOODOO_EFF);
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new sat_dagger_in_dark_big_bad_voodooAI(areaTrigger);
    }
};

// 514. Summoned by 132992 - Gas Bomb
class sat_dagger_in_dark_gas_bomb : public AreaTriggerScript
{
public:
    sat_dagger_in_dark_gas_bomb() : AreaTriggerScript("sat_dagger_in_dark_gas_bomb") { }

    struct sat_dagger_in_dark_gas_bombAI : AreaTriggerAI
    {
        sat_dagger_in_dark_gas_bombAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }
            ...
            if (Player* player = unit->ToPlayer())
                if (player->IsAlive())
                    if (Unit* caster = at->GetCaster())
                        caster->CastSpell(player, SPELL_GAS_BOMB_EFF, true);
        }

        void OnUnitExit(Unit* unit) override
        {
            if (Player* player = unit->ToPlayer())
                player->RemoveAurasDueToSpell(SPELL_GAS_BOMB_EFF);
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new sat_dagger_in_dark_gas_bombAI(areaTrigger);
    }
};

// 515. Summoned by 133005 - Healing Ward
class sat_dagger_in_dark_healing_ward : public AreaTriggerScript
{
public:
    sat_dagger_in_dark_healing_ward() : AreaTriggerScript("sat_dagger_in_dark_healing_ward") { }

    struct sat_dagger_in_dark_healing_wardAI : AreaTriggerAI
    {
        sat_dagger_in_dark_healing_wardAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }
        ...
        void OnUnitEnter(Unit* unit) override
        {
            if (Player* player = unit->ToPlayer())
            {
                ...
                if (Unit* caster = at->GetCaster())
                    caster->CastSpell(player, SPELL_HEALING_WARD_EFF, true);

                // Achievement
                if (Creature* bloodrazor = ObjectAccessor::GetCreature(*player, player->GetInstanceScript() ? player->GetInstanceScript()->GetGuidData(NPC_RAKGOR_BLOODRAZOR + 1) : ObjectGuid::Empty))
                    if (bloodrazor->isInCombat())
                        if (InstanceScript* instance = player->GetInstanceScript())
                            instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_U_MEAN_THATS_NOT_VOID_ZONE), 0);
            }
        }
        ...
        {
            if (Player* player = unit->ToPlayer())
                player->RemoveAurasDueToSpell(SPELL_HEALING_WARD_EFF);
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new sat_dagger_in_dark_healing_wardAI(areaTrigger);
    }
};
...
void AddSC_dagger_in_the_dark()
{
    ...
    RegisterCreatureAI(npc_dagger_in_dark_grizzle_gearslip);
    RegisterCreatureAI(npc_dagger_in_dark_mogu_rune);
    RegisterCreatureAI(npc_dagger_in_dark_darkhatched_shaman);
    RegisterCreatureAI(npc_dagger_in_dark_darkhatched_skulker);
    RegisterCreatureAI(npc_dagger_in_dark_darkhatched_skinflayer);
    RegisterCreatureAI(npc_dagger_in_dark_broodmaster_noshi);
    RegisterCreatureAI(npc_dagger_in_dark_stalactite);
    RegisterCreatureAI(npc_dagger_in_dark_corrupted_dark_hatched_egg);
    RegisterCreatureAI(npc_dagger_in_dark_corrupted_darkhatched_slayer);
    RegisterCreatureAI(npc_dagger_in_dark_darkhatched_oracle);
    RegisterCreatureAI(npc_dagger_in_dark_darkhatched_flesh_hunter);
    RegisterCreatureAI(npc_dagger_in_dark_darkhatched_lizard_lord);
    RegisterCreatureAI(npc_dagger_in_dark_spring_saurok_slayer);
    RegisterCreatureAI(npc_dagger_in_dark_rakgor_bloodrazor);
    RegisterCreatureAI(npc_dagger_in_dark_broken_mogu_tablet);
    RegisterCreatureAI(npc_dagger_in_dark_goblin_cannon);

    RegisterSpellScript(spell_dagger_in_dark_shadowstep_selector);
    RegisterSpellScript(spell_dagger_in_dark_shadowstep_eff);
    RegisterSpellScript(spell_dagger_in_dark_hatch_selector);
    RegisterAuraScript(spell_dagger_in_dark_hatch_eff);
    RegisterSpellScript(spell_dagger_in_dark_death_nova);
    RegisterSpellScript(spell_dagger_in_dark_water_jets);
    RegisterAuraScript(spell_dagger_in_dark_stasis);
    RegisterSpellScript(spell_dagger_in_dark_investigation);
    new sat_dagger_in_dark_mogu_rune_powerup();
    new sat_dagger_in_dark_big_bad_voodoo();
    new sat_dagger_in_dark_gas_bomb();
    new sat_dagger_in_dark_healing_ward();
}
