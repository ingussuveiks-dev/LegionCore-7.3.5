...
enum LittlePatienceTargetPriority
{
    PRIORITY_VICTIM,
    PRIORITY_NOT_VICTIM,
    PRIORITY_CHANNELED,
...
struct LittlePatienceCreatureAI : public ScriptedAI
{
    LittlePatienceCreatureAI(Creature* creature) : ScriptedAI(creature), summons(me) { }
    ...
    void ExecuteTargetEvent(uint32 spellId, uint32 repeat, uint32 eventId, uint32 currentEventId,
        LittlePatienceTargetPriority priority = PRIORITY_VICTIM)
    {
...
struct npc_little_patience_muckmaw_crocolisk : public LittlePatienceCreatureAI
{
    npc_little_patience_muckmaw_crocolisk(Creature* creature) : LittlePatienceCreatureAI(creature) { }
...
struct npc_little_patience_commander_scargash : public LittlePatienceCreatureAI
{
    npc_little_patience_commander_scargash(Creature* creature) : LittlePatienceCreatureAI(creature) { }
...
struct npc_little_patience_krasari_stalker : public LittlePatienceCreatureAI
{
    npc_little_patience_krasari_stalker(Creature* creature) : LittlePatienceCreatureAI(creature) { }
...
struct npc_little_patience_faction_leader : public LittlePatienceCreatureAI
{
    npc_little_patience_faction_leader(Creature* creature) : LittlePatienceCreatureAI(creature) { }
...
struct little_patience_assaulterAI : public LittlePatienceCreatureAI
{
    little_patience_assaulterAI(Creature* creature) : LittlePatienceCreatureAI(creature)
    {
        assaultIndex = 0;
    }
...
// Swamp Gas 68816
struct npc_little_patience_swamp_gas : public LittlePatienceCreatureAI
{
    npc_little_patience_swamp_gas(Creature* creature) : LittlePatienceCreatureAI(creature) { }
...
// Spirit Bomb 68296
struct npc_little_patience_spirit_bomb : public LittlePatienceCreatureAI
{
    npc_little_patience_spirit_bomb(Creature* creature) : LittlePatienceCreatureAI(creature) { }
...
class spell_little_patience_blood_rage : public AuraScript
{
    ...
    void Register() override
    {
        ...
        OnEffectProc += AuraEffectProcFn(spell_little_patience_blood_rage::HandleOnProc, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL);
    }
};
    ...
    void OnUpdate(uint32 /*diff*/, AuraEffect* aurEff)
    {
        ...
        if (Player* owner = GetOwner()->ToPlayer())
            if (Creature* leader = ObjectAccessor::GetCreature(*owner, owner->GetInstanceScript() ? owner->GetInstanceScript()->GetGuidData(leaderEntry) : ObjectGuid::Empty))
                owner->SetPower(POWER_ALTERNATE, leader->AI()->GetData(TYPE_PROGRESS_BAR_VALUE));
    }
...
{
    RegisterCreatureAI(npc_little_patience_muckmaw_crocolisk);
    RegisterCreatureAI(npc_little_patience_commander_scargash);
    RegisterCreatureAI(npc_little_patience_krasari_stalker);
    RegisterCreatureAI(npc_little_patience_king_varian_wrynn);
    RegisterCreatureAI(npc_little_patience_faction_leader);
    RegisterCreatureAI(npc_little_patience_riverblade_marauder);
    RegisterCreatureAI(npc_little_patience_bogrot);
    RegisterCreatureAI(npc_little_patience_swamp_gas);
    RegisterCreatureAI(npc_little_patience_wakened_mogu);
    RegisterCreatureAI(npc_little_patience_spirit_bomb);

    RegisterSpellScript(spell_little_patience_blood_rage_fixate);
    RegisterAuraScript(spell_little_patience_blood_rage);
    RegisterSpellScript(spell_little_patience_inspired_night_elf);
    RegisterSpellScript(spell_little_patience_inspired_jinyu);
    RegisterSpellScript(spell_little_patience_inspired_gnome);
    RegisterSpellScript(spell_little_patience_inspired_dwarf);
    RegisterSpellScript(spell_little_patience_construction_bar_movement);
    RegisterAuraScript(spell_little_patience_faction_construction_progress);
    RegisterAuraScript(spell_little_patience_terrified);
}
