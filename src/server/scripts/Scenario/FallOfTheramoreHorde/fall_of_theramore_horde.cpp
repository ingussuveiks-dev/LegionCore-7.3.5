...
class npc_blastmaster_sparkfuse : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            player->ADD_GOSSIP_ITEM(GossipOptionNpc::None, "Ready the explosives.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
    ...
    void DoAction(int32 actionId) override
    {
        switch (actionId)
        {
            ...
            case ACTION_SONGWEAVER_CHAIN_BREAK:
            {
                ...
                {
                    Talk(TALK_SPECIAL_3);
                    me->SetUInt32Value(UNIT_FIELD_EMOTE_STATE, EMOTE_STATE_CUSTOM_SPELL_01);
                });

                me->AddDelayedEvent(11500, [this]()
                {
                    Talk(TALK_SPECIAL_4);
                });

                me->AddDelayedEvent(14500, [this]()
                {
                    Talk(TALK_SPECIAL_5);
...
class npc_theramore_arcanist : public CreatureScript
{
    public:
        ...
        struct npc_theramore_arcanistAI : public theramore_guard_typeAI
        {
            ...
            {
                SpellInfo const* spell = sSpellMgr->GetSpellInfo(curSpellId);
                if (spell && spell->GetMaxDuration())
                {
                    int32 sDuration = spell->GetMaxDuration();
    ...
    public:
        ...
        struct npc_baldrucAI : public theramore_guard_typeAI
        {
            ...
            void Reset() override
            {
                ...
                summons.DespawnAll();
                if (instance)
                    instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_KITE_FIGHT), 1);
            }
            ...
            {
                if (instance)
                    instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_KITE_FIGHT), 0);
            }
    ...
    public:
        ...
        struct npc_big_bessaAI : public theramore_guard_typeAI
        {
            ...
            void Reset() override
            {
                ...
                if (instance)
                    instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_NO_KITE_YOU), 1);
            }
...
class npc_theramore_thalen_songweaver : public CreatureScript
{
    public:
        ...
        {
            // don`t know real text
            player->ADD_GOSSIP_ITEM(GossipOptionNpc::None, "Use the prison key.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            player->SEND_GOSSIP_MENU(19183, creature->GetGUID());
            return true;
        }
...
class spell_theramore_war_engine : public AuraScript
{
    ...
    void OnAuraEffectApply(AuraEffect const* aurEff, AuraEffectHandleModes mode)
    {
        if (Unit* owner = GetOwner()->ToUnit())
        {
            ...
            owner->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_PULL_TOWARDS, true);
            owner->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_PULL_TOWARDS_DEST, true);
        }
    }
        ...
        if (Creature* caster = GetCaster()->ToCreature())
            if (Player* target = GetHitPlayer())
                if (InstanceScript* instance = caster->GetInstanceScript())
                    instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_NO_KITE_YOU), 0);
    }
...
class scene_theramore_explosion_horde : public SceneTriggerScript
{
    public:
        scene_theramore_explosion_horde() : SceneTriggerScript("scene_theramore_explosion_horde") { }
...
void AddSC_fall_of_theramore_horde_scenario()
{
    ...
    RegisterSpellScript(spell_theramore_rig_to_explode);
    RegisterAuraScript(spell_theramore_war_engine);
    RegisterSpellScript(spell_big_bessa_cannon);
    RegisterSpellScript(spell_big_bessa_cannon_shot);
