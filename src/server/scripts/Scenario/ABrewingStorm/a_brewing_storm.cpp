    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            if (InstanceScript* instance = creature->GetInstanceScript())
            {
                if (instance->GetData(DATA_MAKE_BOOMERS_BREW) < DONE)
                    player->ADD_GOSSIP_ITEM(GossipOptionNpc::None, TEXT_CHAPTER_MAKE_BOOMERS_BREW, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

                if (instance->GetData(DATA_ROAD_TO_THUNDERPAW) < DONE && instance->GetData(DATA_ROAD_TO_THUNDERPAW - 1) == DONE)
                    player->ADD_GOSSIP_ITEM(GossipOptionNpc::None, TEXT_CHAPTER_ROAD_TO_THUNDERPAW, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);

                if (instance->GetData(DATA_SAVE_THUNDERPAW_REFUGE) < DONE && instance->GetData(DATA_SAVE_THUNDERPAW_REFUGE - 1) == DONE)
                    player->ADD_GOSSIP_ITEM(GossipOptionNpc::None, TEXT_CHAPTER_SAVE_THUNDERPAW_REFUGE, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
            }
        ...
        struct npc_brewmaster_blancheAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                if (instance)
                {
                    instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_PERFECT_POUR), 1);
                    instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_PARTY_OF_SIX), 1); // controlled by spells
                }
                me->SetWalk(false);
            }
            ...
            void MovementInform(uint32 type, uint32 pointId) override
            {
                ...
                switch (pointId)
                {
                    ...
                    case 18:
                        Talk(TALK_BREATHING);
                        nonCombatEvents.ScheduleEvent(EVENT_BREATHING, 7500);
                        break;
                    case 34:
                        me->SetFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
        ...
        private:
            void SendBoomersBrewBarToPlayers(bool update = false, bool remove = false)
            {
                ...
                if (update)
                    for (auto&& itr : PlayersInScenario)
                        if (itr->GetPower(POWER_ALTERNATE) < 100)
                            itr->SetPower(POWER_ALTERNATE, itr->GetPower(POWER_ALTERNATE) + 5);
            ...
            bool HasChapterOneCompleted()
            {
                ...
                {
                    if (!SayBrewBar && (itr->GetPower(POWER_ALTERNATE) > 50))
                    {
                        SayBrewBar = true;
                        Talk(TALK_STORM);
                    }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_SWAMP_SMASH:
                    ...
                    events.ScheduleEvent(EVENT_SWAMP_SMASH, urand(7500, 13500));
                    break;
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_WIND_SLASH:
                    ...
                    events.ScheduleEvent(EVENT_WIND_SLASH, 12500);
                    break;
            }
        }
...
{
public:
    sat_blanches_elexir() : AreaTriggerScript("sat_blanches_elexir") { }

    struct sat_blanches_elexirAI : AreaTriggerAI
    {
        sat_blanches_elexirAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }

        void OnUnitEnter(Unit* unit) override
        {
            if (Player* player = unit->ToPlayer())
                if (player->IsAlive())
                    player->CastSpell(player, SPELL_BLANCHES_ELEXIR_EFF, true);
        }

        void OnUnitExit(Unit* unit) override
        {
            if (Player* player = unit->ToPlayer())
                player->RemoveAurasDueToSpell(SPELL_BLANCHES_ELEXIR_EFF);
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new sat_blanches_elexirAI(areaTrigger);
    }
};
...
void AddSC_a_brewing_storm()
{
    ...
    RegisterSpellScript(spell_brewing_storm_lightning_impact);
    RegisterAuraScript(spell_brewing_storm_lightning_channel);
    RegisterSpellScript(spell_boomer_brew_strike);
