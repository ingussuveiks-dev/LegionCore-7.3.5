...
namespace
{
    void PrepareChanneledCast(Creature* creature, float orientation, uint32 spellId)
    {
        creature->AttackStop();
...
struct zanvess_assault_typeAI : public ScriptedAI
{
    ...
    void InitializeAI() override
    {
        ...
        AssaultByType(me->getFaction() == 2550);
    }
            ...
            void DoAction(int32 actionId) override
            {
                switch (actionId)
                {
                    case ACTION_FIRST_TOWER_OFFENSIVE:
                    {
                        ...
                        init.SetFly();
                        init.Launch();
                        events.ScheduleEvent(EVENT_FLY_TO_SECOND_TOWERS, me->movespline->Duration());
                        break;
                    }
                    ...
                    case ACTION_MOVE_TO_ISLAND:
                    {
                        ...
                        init.SetFly();
                        init.Launch();
                        events.ScheduleEvent(EVENT_FLY_TO_ISLAND, me->movespline->Duration());
                        break;
                    }
                }
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_CHAPTER_2:
                        {
                            ...
                            pInit.SetWalk(false);
                            pInit.Launch();
                            events.ScheduleEvent(EVENT_CHAPTER_2, me->movespline->Duration() + 500);
                            break;
                        }
                        case EVENT_FLY_TO_SECOND_TOWERS:
                        {
                            ...
                            pInit.SetWalk(false);
                            pInit.Launch();
                            events.ScheduleEvent(EVENT_FLY_TO_SECOND_TOWERS, me->movespline->Duration() + 500);
                            break;
                        }
                        ...
                        case EVENT_LAUNCH_GUNSHIP:
                        {
                            ...
                            if (instance && instance->GetData(DATA_FACTION))
                            {
                                ...
                                events.ScheduleEvent(EVENT_CHAPTER_2, me->movespline->Duration());
                            }
                            else
                            {
                                // up to sky
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = nonCombatEvents.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_INTRO:
                            Talk(TALK_INTRO);
                            nonCombatEvents.ScheduleEvent(EVENT_SPECIAL_1, 4500);
                            break;
                        ...
                        case EVENT_SPECIAL_5:
                            Talk(TALK_SPECIAL_5);
                            nonCombatEvents.ScheduleEvent(EVENT_SPECIAL_6, 8500);
                            break;
...
class npc_zanvess_sonic_control_tower : public CreatureScript
{
    public:
        ...
        struct npc_zanvess_sonic_control_towerAI : public zanvess_klaxxi_typeAI
        {
            ...
            {
                instance = me->GetInstanceScript();
                nonCombatEvents.ScheduleEvent(EVENT_INIT, 1500);
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    if (eventId == EVENT_STOMP)
                    {
                        ...
                        events.ScheduleEvent(EVENT_STOMP, urand(8500, 13900));
                    }
                    break;
                }
    ...
    public:
        ...
        struct npc_zanvess_team_leader_scooterAI : public zanvess_assault_typeAI
        {
            ...
            void JustDied(Unit* killer) override
            {
                if (instance)
                {
                    instance->SetData(DATA_TEAM_LEADER_DEFEATED, DONE);
                }
            }
                        ...
                        case EVENT_SMASH:
                            if (Unit* vict = me->getVictim())
                            {
                                ...
                                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, NonTankTargetSelector(me)))
                                {
                                    ...
                                    PrepareChanneledCast(me, me->GetAngle(target), SPELL_DEVASTATING_SMASH);
                                }
                                else
                                {
                                    targetGUID = vict->GetGUID();
                                    PrepareChanneledCast(me, me->GetAngle(vict), SPELL_DEVASTATING_SMASH);
                                }
                            }
...
class npc_zanvess_commander_telvrak : public CreatureScript
{
    public:
        ...
        struct npc_zanvess_commander_telvrakAI : public ScriptedAI
        {
            ...
            {
                if (instance)
                    instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_FOR_THE_SWARM), 1);
                summons.DespawnAll();
                events.Reset();
                weaponData = 0;
            }
            ...
            {
                if (instance)
                    instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_FOR_THE_SWARM), 0);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CALL_SWARM:
                            HandleSummonSwarm();
                            events.ScheduleEvent(EVENT_CALL_SWARM, urand(17000, 28500));
                            break;
                    }
                }
    ...
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/, bool /*entered*/) override
        {
            if (InstanceScript* instance = player->GetInstanceScript())
            {
                ...
                if (!instance->GetData(DATA_REACHED_HEART))
                {
                    instance->SetData(DATA_REACHED_HEART, DONE);
...
void AddSC_assault_on_zanvess()
{
    ...
    RegisterCreatureAI(npc_zanvess_scorpid_relocator);
    ...
    RegisterCreatureAI(npc_zanvess_zarthik_guardian);
    RegisterCreatureAI(npc_zanvess_strafing_ran_trigger);

    RegisterAuraScript(spell_zanvess_whirlwind);
    RegisterSpellScript(spell_zanvess_devastating_smash);
    RegisterSpellScript(spell_zanvess_swap_weapon);
    RegisterSpellScript(spell_zanvess_bombard);
