...
class instance_brewmoon_festival : public InstanceMapScript
{
    public:
        ...
        struct instance_brewmoon_festival_InstanceMapScript : public InstanceScript
        {
            ...
            {
                uint8 step = DATA_BREWMOON_FESTIVAL;
                if (chapterOne >= DONE)
                    step = DATA_SCOUTS_REPORT;
                if (chapterTwo >= DONE)
                    step = DATA_YAUNGOL_ATTACK;
                if (chapterThree >= DONE)
                    step = DATA_WARBRINGER_QOBI;
            ...
            void OnCreatureCreate(Creature* creature) override
            {
                switch (creature->GetEntry())
                {
                    case NPC_APOTHECARY_CHENG:
                        ...
                        m_BrewmoonEncounters.emplace(creature->GetEntry(), creature->GetGUID());
                        break;
                ...
                if (unit->ToCreature())
                {
                    switch (unit->GetEntry())
                    {
                        ...
                        case NPC_BATAARI_YAUNGOL:
                        {
                            ...
                            auto scout = m_ScoutsType.find(unit->GetGUID());
                            if (scout != m_ScoutsType.end())
                            {
                                switch (scout->second)
                                {
                                    case TYPE_PASSAGE:
            ...
            void HandleScoutAssault(Position m_pos, uint32 m_type, uint8 m_count, uint32 m_dataType, bool notSave = false)
            {
                for (uint8 i = 0; i < m_count; i++)
                {
                    ...
                    {
                        if (!notSave)
                            m_ScoutsType.emplace(m_temp->GetGUID(), m_type);
                ...
                switch (type)
                {
                    case DATA_BREWMOON_FESTIVAL:
                    {
                        ...
                        if (chapterOne == DONE)
                        {
                            ...
                            SetScenarioStep(DATA_SCOUTS_REPORT);
                    ...
                    case DATA_SCOUTS_REPORT:
                    {
                        ...
                        SetScenarioStep(DATA_YAUNGOL_ATTACK);
                    ...
                    case DATA_YAUNGOL_ATTACK:
                    {
                        ...
                        if (chapterThree >= DONE)
                        {
                            ...
                            SetScenarioStep(DATA_WARBRINGER_QOBI);
            ...
            ObjectGuid GetGuidData(uint32 type) const override
            {
                switch (type)
                {
                    ...
                    case NPC_MISTWEAVER_NIAN:
                    {
                        auto encounter = m_BrewmoonEncounters.find(type);
                        if (encounter != m_BrewmoonEncounters.end())
                            return encounter->second;
                        break;
                    }
                }
            ...
            void Load(char const* in) override
            {
                ...
                if (dataHead1 == 'B' && dataHead2 == 'F')
                {
                    ...
                    if (chapterOne >= DONE)
                    {
                        ...
                        if (chapterTwo < DONE)
                        {
                            m_mEvents.ScheduleEvent(EVENT_SCOUTS, 2 * IN_MILLISECONDS);
                            m_mEvents.ScheduleEvent(EVENT_BLOATS, 4 * IN_MILLISECONDS);
                        }
                    }
                    if (chapterTwo >= DONE)
                    {
                        ...
                        if (chapterThree < DONE)
                        {
                            m_currentType = chapterThree + 1;
                            m_mEvents.ScheduleEvent(EVENT_YAUNGOLS_ATTACK, 2 * IN_MILLISECONDS);
                        }
                    }
                    if (chapterThree >= DONE)
                    {
                    ...
                    if (chapterFour >= DONE)
                        SetBossState(DATA_WARBRINGER_QOBI, DONE);
                }
