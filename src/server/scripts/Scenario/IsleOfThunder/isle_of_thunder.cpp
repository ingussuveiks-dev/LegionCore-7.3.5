...
namespace
{
    void PrepareChanneledCast(Creature* creature, float orientation, uint32 spellId = 0)
    {
        creature->AttackStop();
    ...
    void RemoveChanneledCast(Creature* creature, ObjectGuid targetGuid)
    {
...
// Compatibility loaders preserve the original database script names while
// using LegionCore's script registration interfaces.
template <class AI>
...
class IAreaTriggerOnce
{
public:
    virtual ~IAreaTriggerOnce() = default;
    void Initialize(AreaTrigger* areaTrigger) { _areaTrigger = areaTrigger; }
    bool CanTrigger(WorldObject* object) { return CheckTriggering(object); }
    void Trigger(WorldObject* object) { OnTrigger(object); }
...
public:
    ...
    struct AI : AreaTriggerAI
    {
        ...
        void OnUnitEnter(Unit* unit) override
        {
            ...
            if constexpr (std::is_base_of<IAreaTriggerAura, Handler>::value)
                handler.Apply(unit);
            else
            {
                handler.Trigger(unit);
                at->Remove();
            }
        }
...
// The Pandaria source used a project-specific combat helper. Keep the
// compatibility layer local to Isle of Thunder instead of changing LegionCore.
enum IsleOfThunderTargetPriority
{
    PRIORITY_VICTIM,
    PRIORITY_NOT_VICTIM,
    PRIORITY_CHANNELED,
...
struct customCreatureAI : public ScriptedAI
{
    ...
    void ExecuteTargetEvent(uint32 spellId, uint32 repeat, uint32 eventId, uint32 currentEventId,
        IsleOfThunderTargetPriority priority = PRIORITY_VICTIM)
    {
        ...
        if (target)
            DoCast(target, spellId, spellId == 129040);
...
class npc_shaolmara_lady_jaina : public CreatureScript
{
    public:
        ...
        {
            if (creature->HasFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER) && creature->GetMapId() == 1064)
    ...
    void DoAction(int32 actionId) override
    {
        if (actionId == ACTION_START_INTRO)
        {
            ...
            scheduler.Schedule(Milliseconds(1500), [this](TaskContext context)
            {
                ...
                uint32 delay = me->GetEntry() == NPC_VEERISA_HIPPOGRYPH ? me->movespline->Duration() : me->movespline->Duration() - rand() % 8;
                scheduler.Schedule(Milliseconds(delay), [this](TaskContext context)
                {
                    DoCast(me, SPELL_LIGHTNING_STRIKE, true);
                });
        ...
        switch (actionId)
        {
            ...
            case ACTION_AMBUSH_PREVENTED:
                ...
                scheduler.Schedule(Milliseconds(delay += 5 * IN_MILLISECONDS), [this](TaskContext context)
                {
                    ...
                    scheduler.Schedule(Milliseconds(me->movespline->Duration() + 3 * IN_MILLISECONDS), [this](TaskContext context)
                    {
                        thirdStep = false;
            ...
            case ACTION_ANKI_DEFEAT:
            {
                ...
                events.ScheduleEvent(EVENT_MISC + 3, me->movespline->Duration());
                break;
            }
            case ACTION_BEHIND_ANKI:
            {
                ...
                init.SetUncompressed();
                init.Launch();
                events.ScheduleEvent(EVENT_MISC, me->movespline->Duration());
                break;
            }
    ...
    void DoAction(int32 actionId) override
    {
        if (actionId == ACTION_START_INTRO)
        {
            ...
            scheduler.Schedule(Milliseconds(1500), [this](TaskContext context)
            {
                ...
                uint32 delay = me->GetEntry() == NPC_ELSIA_BLOODHAWK ? me->movespline->Duration() : me->movespline->Duration() - rand() % 8;
                scheduler.Schedule(Milliseconds(delay), [this](TaskContext context)
                {
                    DoCast(me, SPELL_LIGHTNING_STRIKE, true);
                });
...
class npc_teardown_scout_captain_elsia : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            player->ADD_GOSSIP_ITEM(GossipOptionNpc::None, "I`m ready", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
...
struct npc_beastmaster_horaki : public customCreatureAI
{
    ...
    void DoAction(int32 actionId) override
    {
        switch (actionId)
        {
            ...
            case ACTION_FLY_FALL:
                if (Player* target = ObjectAccessor::GetPlayer(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(PLAYER_DATA) : ObjectGuid::Empty))
                    Talk(TALK_SPECIAL_1, target->GetGUID());

                scheduler
                    .Schedule(Seconds(4), [this](TaskContext context)
                {
                    if (Player* target = ObjectAccessor::GetPlayer(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(PLAYER_DATA) : ObjectGuid::Empty))
                        Talk(TALK_SPECIAL_2, target->GetGUID());
                });
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        if (Player* target = ObjectAccessor::GetPlayer(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(PLAYER_DATA) : ObjectGuid::Empty))
            Talk(TALK_SPECIAL_3, target->GetGUID());
        ...
        struct npc_taoshi_to_skiesAI : public customCreatureAI
        {
            ...
            void DoAction(int32 actionId) override
            {
                if (actionId == ACTION_BREAK_THE_WALL)
                {
                    ...
                    scheduler
                        .Schedule(Milliseconds(me->movespline->Duration() + 5 * IN_MILLISECONDS), [this](TaskContext context)
                    {
                        if (me->GetInstanceScript())
                        {
...
struct npc_thunderwing_stormsea_landing : public customCreatureAI
{
    ...
    void OnSpellClick(Unit* clicker) override
    {
        if (Creature* taoshi = ObjectAccessor::GetCreature(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(NPC_TAOSHI_STORMSEA) : ObjectGuid::Empty))
        {
            ...
            int32 bp = 2;
            taoshi->CastCustomSpell(VEHICLE_SPELL_RIDE_HARDCODED, SPELLVALUE_BASE_POINT0, bp, me, true);
        }
            ...
            .Schedule(Milliseconds(me->movespline->Duration()), [this](TaskContext context)
        {
                ...
                .Schedule(Milliseconds(3500), [this](TaskContext context)
            {
                ...
                uint32 delay = 0;
                scheduler
                    .Schedule(Milliseconds(delay += me->movespline->Duration() / 2), [this](TaskContext context)
                {
                    if (Creature* shanbu = ObjectAccessor::GetCreature(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(NPC_SHAN_BU_STORMSEA) : ObjectGuid::Empty))
    ...
    void DoAction(int32 actionId) override
    {
        if (actionId == ACTION_START_INTRO)
        {
                ...
                .Schedule(Milliseconds(delay += 3500), [this](TaskContext context)
            {
                ...
                if (Unit* target = ObjectAccessor::GetUnit(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(PLAYER_DATA) : ObjectGuid::Empty))
                    RemoveChanneledCast(me, target->GetGUID());
            });
        }
    }
...
struct npc_huseng_gatekeeper : public customCreatureAI
{
    ...
    {
        summon->SetDisplayId(sObjectMgr->GetCreatureDisplay(summon->GetCreatureTemplate()->Modelid[1]));
        ...
        if (Unit* target = ObjectAccessor::GetUnit(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(PLAYER_DATA) : ObjectGuid::Empty))
        {
            ...
            PrepareChanneledCast(summon, summon->GetOrientation());
        ...
        switch (actionId)
        {
            ...
            case ACTION_KEEP_TO_SHANBU:
                    ...
                    .Schedule(Milliseconds(delay += 5500), [this](TaskContext context)
                {
                    ...
                    delay = 0;
                    scheduler
                        .Schedule(Milliseconds(delay += me->movespline->Duration() + 2000), [this](TaskContext context)
                    {
                        Talk(TALK_SPECIAL_4);
                    });
...
struct npc_shanbu_nalak : public customCreatureAI
{
    ...
    void DoAction(int32 actionId) override
    {
        if (actionId == ACTION_START_INTRO)
        {
            ...
            scheduler
                .Schedule(Milliseconds(me->movespline->Duration() / 2), [this](TaskContext context)
            {
                Talk(TALK_INTRO);
            });
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            if (creature->GetInstanceScript())
            {
                if (creature->GetInstanceScript()->GetData(STEP_IN_FORGE) == DONE)
                    player->ADD_GOSSIP_ITEM(GossipOptionNpc::None, "I`m ready!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
                else
                    player->ADD_GOSSIP_ITEM(GossipOptionNpc::None, "I`m ready!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            }
            else if (player->GetQuestStatus(QUEST_THE_THUNDER_FORGE) == QUEST_STATUS_INCOMPLETE)
                player->ADD_GOSSIP_ITEM(GossipOptionNpc::None, "I`m ready!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
...
struct npc_thunder_forge_celestial_blacksmith : public ScriptedAI
{
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        ...
        hasBegun = false;
        me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PACIFIED);
        PrepareChanneledCast(me, me->GetOrientation());
...
// Launch Tear Down This Wall intro scene 137570. LegionCore exposes scene
// completion through SceneTriggerScript, so preserve the original start-time
// teleport on the launcher spell itself.
class spell_iot_tear_down_wall_intro : public SpellScript
{
    PrepareSpellScript(spell_iot_tear_down_wall_intro);

    void HandleAfterCast()
    {
        if (Player* player = GetCaster()->ToPlayer())
            player->NearTeleportTo(tearDownPastIntro.GetPositionX(), tearDownPastIntro.GetPositionY(),
                tearDownPastIntro.GetPositionZ(), tearDownPastIntro.GetOrientation());
    }

    void Register() override
    {
        AfterCast += SpellCastFn(spell_iot_tear_down_wall_intro::HandleAfterCast);
    }
};
...
class spell_teardown_legacy_of_blood : public AuraScript
{
    ...
    void HandleOnProc(AuraEffect const* /*auraEffect*/, ProcEventInfo& eventInfo)
    {
        if (Unit* owner = GetOwner()->ToUnit())
        {
            int32 damage = int32(eventInfo.GetDamageInfo()->GetDamage());
            owner->CastCustomSpell(SPELL_LEGACY_OF_BLOOD_EFF, SPELLVALUE_BASE_POINT0, damage, owner, true);
        }
    }

    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_teardown_legacy_of_blood::HandleOnProc, EFFECT_FIRST_FOUND, SPELL_AURA_ANY);
    }
};
...
// SceneId 169
class scene_isle_of_thunder_tear_down_wall_intro : public SceneTriggerScript
{
    public:
        scene_isle_of_thunder_tear_down_wall_intro() : SceneTriggerScript("scene_isle_of_thunder_tear_down_wall_intro") { }

        bool OnTrigger(Player* player, SpellScene const* /*scene*/, std::string triggerName) override
        {
            if (triggerName != "complete")
                return false;
...
// SceneId 167
class scene_isle_of_thunder_tear_down_wall_explosive : public SceneTriggerScript
{
    public:
        scene_isle_of_thunder_tear_down_wall_explosive() : SceneTriggerScript("scene_isle_of_thunder_tear_down_wall_explosive") { }
...
void AddSC_scenario_isle_of_thunder()
{
    ...
    new spell_script<spell_iot_tear_down_wall_intro>("spell_iot_tear_down_wall_intro");
