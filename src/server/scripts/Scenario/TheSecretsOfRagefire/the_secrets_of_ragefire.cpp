...
enum RagefireTargetPriority
{
    PRIORITY_VICTIM,
    PRIORITY_SELF,
};

struct RagefireCreatureAI : public ScriptedAI
{
    RagefireCreatureAI(Creature* creature) : ScriptedAI(creature) { }

    void ExecuteTargetEvent(uint32 spellId, uint32 repeat, uint32 eventId, uint32 currentEventId,
        RagefireTargetPriority priority = PRIORITY_VICTIM)
    {
        ...
        switch (actionId)
        {
            case ACTION_START_INTRO:
                    ...
                    .Schedule(Milliseconds(delay += 2500), [this](TaskContext context)
                {
                    if (Player* owner = ObjectAccessor::GetPlayer(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(PLAYER_DATA) : ObjectGuid::Empty))
                        Talk(TALK_SPECIAL_2, owner->GetGUID());
                });
                    ...
                    .Schedule(Milliseconds(delay += 6100), [this](TaskContext context)
                {
                    ...
                    Movement::MoveSplineInit init(*newt);
                            ...
                            scheduler
                                .Schedule(Milliseconds(newt->movespline->Duration()), [this](TaskContext context)
                            {
                                if (Creature* newt = ObjectAccessor::GetCreature(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(NPC_NEWT) : ObjectGuid::Empty))
            ...
            case ACTION_INNER_RAGEFIRE:
            {
                ...
                if (me->GetInstanceScript())
                    me->GetInstanceScript()->SetData(CRITERIA_REACH_RAGEFIRE, DONE);
            ...
            {
                if (Player* owner = ObjectAccessor::GetPlayer(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(PLAYER_DATA) : ObjectGuid::Empty))
                    Talk(TALK_SPECIAL_14, owner->GetGUID());
            ...
            case ACTION_REACH_SEALED_GATES:
            {
                ...
                if (me->GetInstanceScript())
                    me->GetInstanceScript()->SetData(CRITERIA_MEET_WITH_TICKER, DONE);
                    ...
                    if (Creature* portalController = ObjectAccessor::GetCreature(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(NPC_EMERGENCY_TELEPORTER) : ObjectGuid::Empty))
                    {
                        for (auto&& itr : goblinsList)
                        {
                            if (Creature* gobSquad = ObjectAccessor::GetCreature(*me, me->GetInstanceScript() ? me->GetInstanceScript()->GetGuidData(itr) : ObjectGuid::Empty))
                            {
                                ...
                                float z = gobSquad->GetPositionZ();
                                gobSquad->UpdateAllowedPositionZ(x, y, z);
                                gobSquad->GetMotionMaster()->MoveJump(x, y, z, 0.0f, 25.0f, 25.0f);
                            }
                        }
                    }
                });
    ...
    void OnSpellClick(Unit* clicker) override
    {
        ...
        for (auto&& itr : bunnyList)
        {
            ...
            if (Creature* bomb = me->FindNearestCreature(NPC_BOMB, 50.0f, true))
            {
                ...
                if (!moveDuration)
                    moveDuration = itr->movespline->Duration();
            }
        }
...
struct npc_ragefire_dark_shaman_xorenth : public RagefireCreatureAI
{
    npc_ragefire_dark_shaman_xorenth(Creature* creature) : RagefireCreatureAI(creature)
    {
        korkronDied = 0;
        delay = 0;
        x = 0.0f; y = 0.0f;
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        ...
        events.ScheduleEvent(EVENT_RUINED_EARTH, 16500);
    }
        ...
        if (actionId == ACTION_START_INTRO && ++korkronDied > 5 && me->GetInstanceScript() && me->GetInstanceScript()->GetData(DATA_INFILTRATION) != DONE)
        {
                ...
                .Schedule(Milliseconds(delay), [this](TaskContext context)
            {
                ...
                for (auto&& itr : peonsList)
                {
                    ...
                    init.Launch();
                    itr->DespawnOrUnsummon(itr->movespline->Duration());
                }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            if (eventId == EVENT_RUINED_EARTH) // Announce
                Talk(TALK_SPECIAL_5);

            ExecuteTargetEvent(SPELL_TWISTED_ELEMENTS, urand(2600, 3000), EVENT_TWISTED_ELEMENTS, eventId);
            ExecuteTargetEvent(SPELL_GLACIAL_FREEZE_TOTEM, 16500, EVENT_GLACIAL_FREEZE_TOTEM, eventId, PRIORITY_SELF);
...
struct npc_ragefire_korkron_dark_shaman : public RagefireCreatureAI
{
    npc_ragefire_korkron_dark_shaman(Creature* creature) : RagefireCreatureAI(creature) { }
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        ...
        events.ScheduleEvent(EVENT_POISON_BOLT_TOTEM, urand(8000, 12500));
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ExecuteTargetEvent(SPELL_LAVA_BURST, urand(2100, 2500), EVENT_LAVA_BURST, eventId);
            ExecuteTargetEvent(SPELL_POISON_BOLT_TOTEM, 16500, EVENT_POISON_BOLT_TOTEM, eventId, PRIORITY_SELF);
            break;
        }
...
struct npc_ragefire_korkron_emberguard : public RagefireCreatureAI
{
    npc_ragefire_korkron_emberguard(Creature* creature) : RagefireCreatureAI(creature) { }
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        events.ScheduleEvent(EVENT_EMBERSTRIKES, urand(6500, 12500));
    }
...
struct npc_ragefire_investigated_item : public ScriptedAI
{
    ...
    void Reset() override
    {
        if (uint32 displayId = me->GetCreatureTemplate()->Modelid[1])
            me->SetDisplayId(sObjectMgr->GetCreatureDisplay(displayId));
    ...
    void OnSpellClick(Unit* clicker) override
    {
        ...
        if (me->GetInstanceScript())
        {
            me->GetInstanceScript()->SetData(investigatedItems.find(me->GetEntry())->second, DONE);
        }
...
struct npc_ragefire_korkron_shadowblade : public RagefireCreatureAI
{
    npc_ragefire_korkron_shadowblade(Creature* creature) : RagefireCreatureAI(creature) { }
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        events.ScheduleEvent(EVENT_SHADOWSTEP, urand(6500, 12500));
    }
...
struct npc_ragefire_overseer_elaglo : public RagefireCreatureAI
{
    npc_ragefire_overseer_elaglo(Creature* creature) : RagefireCreatureAI(creature)
    {
        x = 0.0f; y = 0.0f;
    }
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        events.ScheduleEvent(EVENT_DEMOLISH_ARMOR, urand(8000, 12500));
        events.ScheduleEvent(EVENT_SHATTERING_STOMP, 6500);
    }
                ...
                .Schedule(Milliseconds(3600), [this](TaskContext context)
            {
                ...
                for (auto&& itr : invKorkronAssault1)
                {
                    if (Creature* assault = me->SummonCreature(itr.first, itr.second, TEMPSUMMON_MANUAL_DESPAWN))
                    {
                        ...
                        assault->GetMotionMaster()->MoveJump(x, y, z, 0.0f, 20.0f, 20.0f, EVENT_JUMP);
                    }
                }
            });
        }
    ...
    void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
    {
        ...
        if (summonerGUIDs.empty())
        {
            ...
            if (waveId > 3) // Elaglo has arrived in combat
            {
                ...
                me->GetMotionMaster()->MoveJump(x, y, z, 0.0f, 20.0f, 20.0f, EVENT_JUMP);
                return;
            }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_DEMOLISH_ARMOR:
                    ...
                    events.ScheduleEvent(EVENT_DEMOLISH_ARMOR, urand(6500, 12500));
                    break;
...
struct npc_ragefire_korkron_dire_solder : public RagefireCreatureAI
{
    npc_ragefire_korkron_dire_solder(Creature* creature) : RagefireCreatureAI(creature) { }
...
// Flame Hound 70702
struct npc_ragefire_flame_hound : public RagefireCreatureAI
{
    npc_ragefire_flame_hound(Creature* creature) : RagefireCreatureAI(creature) { }
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        events.ScheduleEvent(EVENT_FLAME_BREATH, urand(4500, 8000));
    }
...
class sat_ragefire_emberstrikes : public AreaTriggerScript
{
public:
    sat_ragefire_emberstrikes() : AreaTriggerScript("sat_ragefire_emberstrikes") { }

    struct sat_ragefire_emberstrikesAI : AreaTriggerAI
    {
        sat_ragefire_emberstrikesAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }
        ...
        void OnUnitEnter(Unit* unit) override
        {
            ...
            {
                if (player->IsAlive())
                    caster->CastSpell(player, SPELL_POOL_OF_EMBERS, true);
                return;
            }

            Creature* creature = unit->ToCreature();
            if (!creature || !IsGobSquad(creature->GetEntry()))
                return;

            // The squad uses friendly faction in the sniff, so record the
            // achievement failure without applying hostile spell damage.
            creature->SetHealth(uint32(creature->GetMaxHealth() * 0.9f));
            if (InstanceScript* instance = creature->GetInstanceScript())
                instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_FEW_PROUD_GOB_SQUAD), 0);
        }

        void OnUnitExit(Unit* unit) override
        {
            unit->RemoveAurasDueToSpell(SPELL_POOL_OF_EMBERS);
        }

        bool IsGobSquad(uint32 entry) const
        {
            return std::find(goblinsList.begin(), goblinsList.end(), entry) != goblinsList.end();
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new sat_ragefire_emberstrikesAI(areaTrigger);
    }
};
...
{
    RegisterCreatureAI(npc_ragefire_grit);
    RegisterCreatureAI(npc_ragefire_detonator);
    RegisterCreatureAI(npc_ragefire_dark_shaman_xorenth);
    RegisterCreatureAI(npc_ragefire_ruined_earth);
    RegisterCreatureAI(npc_ragefire_korkron_dark_shaman);
    RegisterCreatureAI(npc_ragefire_korkron_emberguard);
    RegisterCreatureAI(npc_ragefire_investigated_item);
    RegisterCreatureAI(npc_ragefire_korkron_shadowblade);
    RegisterCreatureAI(npc_ragefire_bring_items);
    RegisterCreatureAI(npc_ragefire_overseer_elaglo);
    RegisterCreatureAI(npc_ragefire_korkron_dire_solder);
    RegisterCreatureAI(npc_ragefire_poison_bolt_totem);
    RegisterCreatureAI(npc_ragefire_flame_hound);
    RegisterCreatureAI(npc_ragefire_shattered_earth);

    RegisterSpellScript(spell_ragefire_ruined_earth_selector);
    new sat_ragefire_emberstrikes();
