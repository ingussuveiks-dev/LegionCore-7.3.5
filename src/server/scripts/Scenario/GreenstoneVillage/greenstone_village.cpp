    ...
    public:
        ...
        struct npc_greenstone_village_ownerAI : public ScriptedAI
        {
            ...
            void MovementInform(uint32 type, uint32 pointId) override
            {
                ...
                {
                    Talk(TALK_SPECIAL_2);
                    me->SetUInt32Value(UNIT_FIELD_EMOTE_STATE, EMOTE_STATE_SIT_CHAIR_LOW);
                }
            }
...
{
    public:
        npc_greenstone_belligerent_blossom() : CreatureScript("npc_greenstone_belligerent_blossom") { }
        ...
        struct npc_greenstone_village_burgled_barrelAI : public ScriptedAI
        {
            ...
            void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/) override
            {
                if (!hasAchievFail)
                {
                    hasAchievFail = true;
                    if (instance)
                        instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_PERFECT_DELIVERY), 0);
                }
            }
        ...
        struct npc_greenstone_vengeful_huiAI : public ScriptedAI
        {
            ...
            void DoAction(int32 actionId) override
            {
                if (actionId == ACTION_INTRO)
                {
                    ...
                    nonCombatEvents.ScheduleEvent(EVENT_INTRO, duration);
                }
            }
...
class spell_greenstone_volatile_greenstone_brew : public SpellScript
{
    ...
    {
        if (InstanceScript* instance = GetCaster()->GetInstanceScript())
            instance->DoUpdateWorldState(static_cast<WorldStates>(WORLDSTATE_SAVE_IT_FOR_LATER), 0);
    }
...
void AddSC_greenstone_village()
{
    ...
    RegisterSpellScript(spell_greenstone_volatile_greenstone_brew);
