...
struct npc_unga_bird_haver : public ScriptedAI
{
    ...
    void DoAction(int32 actionId) override
    {
        if (actionId == ACTION_INTRO && ++m_bcount >= 8)
        {
            ...
            events.ScheduleEvent(EVENT_KNIFE_FIGHT, 5500);
        }
    }
        ...
        if (actionId == ACTION_INTRO)
        {
            switch (me->GetEntry())
            {
                ...
                case NPC_JUNGLE_BREWSTEALER:
                {
                    ...
                    init.SetUncompressed();
                    init.Launch();
                    nonCombatEvents.ScheduleEvent(EVENT_COSMETIC, me->movespline->Duration() + 1 * IN_MILLISECONDS);
                    break;
                }
...
class spell_unga_throw_keg : public SpellScript
{
    ...
    void HandleAfterCast()
    {
        if (Creature* caster = GetCaster()->ToCreature())
            caster->SetUInt32Value(UNIT_FIELD_EMOTE_STATE, EMOTE_STATE_SPELL_CHANNEL_OMNI);
    }
    ...
    void HandleAuraEffectApply(AuraEffect const* aurEff, AuraEffectHandleModes mode)
    {
        if (Unit* owner = GetOwner()->ToUnit())
            if (Creature* brewMaster = ObjectAccessor::GetCreature(*owner, owner->GetInstanceScript() ? owner->GetInstanceScript()->GetGuidData(NPC_BREWMASTER_BO_BREW) : ObjectGuid::Empty))
                brewMaster->AI()->Talk(TALK_SPECIAL_17, owner->GetGUID());
    }
...
class sat_bo_rejuvinating_mist : public AreaTriggerScript
{
public:
    sat_bo_rejuvinating_mist() : AreaTriggerScript("sat_bo_rejuvinating_mist") { }

    struct sat_bo_rejuvinating_mistAI : AreaTriggerAI
    {
        sat_bo_rejuvinating_mistAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }

        void OnUnitEnter(Unit* unit) override
        {
            if (Player* player = unit->ToPlayer())
                if (player->IsAlive())
                    player->CastSpell(player, SPELL_RENEWING_MIST, true);
        }

        void OnUnitExit(Unit* unit) override
        {
            if (Player* player = unit->ToPlayer())
                player->RemoveAurasDueToSpell(SPELL_RENEWING_MIST);
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new sat_bo_rejuvinating_mistAI(areaTrigger);
    }
};
...
void AddSC_unga_ingoo()
{
    ...
    RegisterCreatureAI(npc_unga_spearscamp);
    RegisterCreatureAI(npc_unga_bird_haver);
    RegisterCreatureAI(npc_unga_gladiators);
    RegisterCreatureAI(npc_unga_zipline_to_ship);
    RegisterCreatureAI(npc_unga_captain_ook);
    RegisterCreatureAI(npc_unga_banana_ship);
    RegisterCreatureAI(npc_unga_pirate);
    RegisterCreatureAI(npc_unstable_keg);
    RegisterCreatureAI(npc_unga_ookie);
    new go_brew_container();

    RegisterAuraScript(spell_calling_birds);
    RegisterSpellScript(spell_unga_throw_keg);
    RegisterSpellScript(spell_unga_mug_collected);
    RegisterAuraScript(spell_unga_jungle_brew_collected);
    RegisterAuraScript(spell_unga_encumbered);
    RegisterAuraScript(spell_unga_popped_cork);
    new AreaTrigger_at_unga_ingoo_cauldron();
    new sat_bo_rejuvinating_mist();
}
