...
float const LB_MAX_ENGAGE_DISTANCE = 30.0f;
    ...
    void UnregisterBotSession(ObjectGuid botGuid)
    {
        ...
        g_legionBotLfgState.erase(botGuid);
        g_legionBotTargetSince.erase(botGuid);
    }
    ...
    void EquipBotGear(Player* bot, uint8 level)
    {
        ...
        for (uint32 itemId : GetBotGearForLevel(bot, level))
        {
            ...
            {
                TC_LOG_ERROR("scripts.custom.legionbot", "PlayerBot: skipping unknown gear item %u for %s", itemId, bot->GetName());
                continue;
            }
    ...
    void DestroyBotPlayer(ObjectGuid botGuid, WorldSessionPtr session)
    {
        ...
        TC_LOG_INFO("scripts.custom.legionbot", "PlayerBot: dismissing bot %u", botGuid.GetCounter());
...
void LegionBot_Spawn(Player* owner, std::string const& charName, ChatHandler* handler, uint8 role = 255)
{
    ...
    // Only characters on the designated bot account may be loaded by a player command.
    uint32 const botAccountId = uint32(sConfigMgr->GetIntDefault("LegionBot.AccountId", 0));
    if (!botAccountId || accountId != botAccountId)
    {
        if (handler)
        {
            handler->SendSysMessage("|cffff4444Playerbot:|r character is not on the configured bot account.");
            handler->SetSentErrorMessage(true);
        }
        return;
    }

    ObjectGuid const botGuid = ObjectGuid::Create<HighGuid::Player>(lowGuid);
    if (ObjectAccessor::FindPlayer(botGuid))
    {
        if (handler)
        {
            handler->SendSysMessage("|cffff4444Playerbot:|r this character is already online.");
            handler->SetSentErrorMessage(true);
        }
        return;
    }

    if (LegionBot_IsBot(botGuid))
    {
    ...
    auto session = std::make_shared<WorldSession>(accountId, std::string(safeName), nullptr, SEC_PLAYER, 6, 0, "bot",
        LOCALE_enUS, 0, false, AT_AUTH_FLAG_NONE, std::unordered_map<uint8, int64>());
    {
        std::lock_guard<std::mutex> lock(g_legionBotsMutex);
    ...
    auto holder = std::make_shared<LoginQueryHolder>(accountId, ObjectGuid::Create<HighGuid::Player>(lowGuid));
    ...
    if (!placed)
    {
        Position const failedPosition = bot->GetPosition();
        TC_LOG_ERROR("scripts.custom.legionbot", "PlayerBot: AddPlayerToMap failed for %s (map %u inst %u pos %.1f %.1f %.1f ownerMap %u ownerInst %u)",
        ...
        if (handler)
        {
            handler->PSendSysMessage("|cffff4444LegionBot:|r failed to place '%s' (map %u inst %u pos %.1f %.1f %.1f).", safeName.c_str(),
                map->GetId(), map->GetInstanceId(), failedPosition.GetPositionX(), failedPosition.GetPositionY(), failedPosition.GetPositionZ());
            handler->SetSentErrorMessage(true);
        }
        return;
    }
    ...
    if (!bot->IsInWorld())
    {
        TC_LOG_ERROR("scripts.custom.legionbot", "PlayerBot: %s not in world after AddPlayerToMap (map %u)", safeName.c_str(), map->GetId());
        session->SetPlayer(nullptr);
        UnregisterBotSession(ObjectGuid::Create<HighGuid::Player>(lowGuid));
        if (bot->IsInGrid())
            map->RemovePlayerFromMap(bot, false);
...
void LegionBot_OnPlayerUpdate(Player* player, uint32 /*diff*/)
{
    ...
    for (ObjectGuid botGuid : botGuids)
    {
        ...
        {
            lfg::LfgState playerLfgState = sLFGMgr->GetState(bot->GetGUID(), sLFGMgr->GetQueueId(bot->GetGUID()));
...
void LegionBot_OnPlayerLogout(Player* player)
{
    ...
    g_legionBotSettings.erase(player->GetGUID());
    g_legionBotLastLevelCheck.erase(player->GetGUID());
}
