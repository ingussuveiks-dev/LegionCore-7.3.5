...
{
	TalkAggro = 0,        ///< Rakun, lets give'em hell! [45878]
            ...
            if (m_Intro)
            {
                if (m_Instance != nullptr)
                {
                    if (Creature* l_Dragon = m_Instance->instance->GetCreature(m_Instance->GetGuidData(GrimrailDepotData::DataSkyLordTovraDragon)))
                    {
                        ...
                        me->GetAreaTriggerListWithEntryInGrid(l_AreatriggersList, 2324, 200.0f);
                        me->GetAreaTriggerListWithEntryInGrid(l_AreatriggersList, 2360, 200.0f);
                ...
                if (m_Count >= 3)
                {
                    ...
                    if (m_Instance != nullptr)
                    {
                        if (Creature* l_Dragon = m_Instance->instance->GetCreature(m_Instance->GetGuidData(GrimrailDepotData::DataSkyLordTovraDragon)))
                        {
                            ...
                            l_Dragon->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_REMOVE_CLIENT_CONTROL);
            ...
            scheduler.Schedule(Seconds(4), [this](TaskContext /*context*/)
            {
                ...
                if (InstanceScript* m_Instance = me->GetInstanceScript())
                {
                    if (Creature* l_Dragon = m_Instance->instance->GetCreature(m_Instance->GetGuidData(GrimrailDepotData::DataSkyLordTovraDragon)))
                    {
                        ...
                        me->GetPlayerListInGrid(l_ListPlayers, 600.0f);
                        if (!l_ListPlayers.empty())
                        {
                            for (Player* l_Itr : l_ListPlayers)
                            {
                                ...
                                l_Itr->SendSpellScene(GrimrailDepotScenes::SceneEscapeTheTrain, nullptr, true, l_Itr);
                            }
                        }
                    }
                }
...
class grimrail_depot_skylord_tovra_creature_rkuna : public CreatureScript
{
    ...
    struct grimrail_depot_skylord_tovra_creature_rkunaAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            me->SetAnimKitId(eSkylordTorvaOneShotAnimKit::AnimKitFly);
        ...
        void DoAction(int32 const p_Action) override
        {
            ...
            case eSkylordTorvaActions::ActionCombatStartDragonFlight:
                ...
                {
                    m_Flight = false;
                    me->GetMotionMaster()->MoveTakeoff(eSkylordTorvaMovements::MovementFlyPoint, g_DrakePointA.GetPositionX(), g_DrakePointA.GetPositionY(), g_DrakePointA.GetPositionZ());
                }
                else
                {
                    m_Flight = true;
                    me->GetMotionMaster()->MoveTakeoff(eSkylordTorvaMovements::MovementFlyPoint, g_DrakePointB.GetPositionX(), g_DrakePointB.GetPositionY(), g_DrakePointB.GetPositionZ());
                }
            ...
            case eSkylordTorvaActions::ActionCombatStopDragonFlight:
                ...
                me->GetMotionMaster()->MoveTakeoff(0, me->GetHomePosition().GetPositionX(), me->GetHomePosition().GetPositionY(), me->GetHomePosition().GetPositionZ());
                break;
            default:
                break;
            }
            ...
            case eSkylordTorvaMovements::MovementTorvaHomePosition:
                if (m_Instance != nullptr)
                {
                    if (Creature* l_SkylordTorva = m_Instance->instance->GetCreature(m_Instance->GetGuidData(GrimrailDepotData::DataSkyLordTovra)))
                    {
                        ...
                        l_SkylordTorva->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_IMMUNE_TO_PC);
        ...
        {
            if (p_Unit && p_Unit->GetTypeId() == TypeID::TYPEID_PLAYER && p_Unit->IsWithinDistInMap(me, 40.0f) && !m_First && m_Count >= 5)
            {
                m_First = true;
...
class grimrail_depot_skylord_tovra_at_diffused_energy : public AreaTriggerScript
{
public:
    grimrail_depot_skylord_tovra_at_diffused_energy() : AreaTriggerScript("grimrail_depot_skylord_tovra_at_diffused_energy") { }

    struct grimrail_depot_skylord_tovra_at_diffused_energyAI : AreaTriggerAI
    {
        grimrail_depot_skylord_tovra_at_diffused_energyAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }

        void OnUpdate(uint32 /*diff*/) override
        {
            std::list<Unit*> targets;
            at->GetAttackableUnitListInRange(targets, 2.0f);
            for (Unit* target : targets)
                if (!target->HasAura(161588))
                    target->CastSpell(target, 161588, true);
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new grimrail_depot_skylord_tovra_at_diffused_energyAI(areaTrigger);
    }
};

/// Spinning Spear - 162058
class grimrail_depot_skylord_tovra_at_spinning_spear : public AreaTriggerScript
{
public:
    grimrail_depot_skylord_tovra_at_spinning_spear() : AreaTriggerScript("grimrail_depot_skylord_tovra_at_spinning_spear") { }

    struct grimrail_depot_skylord_tovra_at_spinning_spearAI : AreaTriggerAI
    {
        grimrail_depot_skylord_tovra_at_spinning_spearAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }
        uint32 timer = 500;
        ...
        void OnUpdate(uint32 diff) override
        {
            ...
            std::list<Unit*> targets;
            at->GetAttackableUnitListInRange(targets, 3.0f);
            for (Unit* target : targets)
                target->CastSpell(target, 162057, true);
            timer = IN_MILLISECONDS;
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new grimrail_depot_skylord_tovra_at_spinning_spearAI(areaTrigger);
    }
};

/// Freezing Snare - 162059
class grimrail_depot_skylord_torva_at_freezing_snare : public AreaTriggerScript
{
public:
    grimrail_depot_skylord_torva_at_freezing_snare() : AreaTriggerScript("grimrail_depot_skylord_torva_at_freezing_snare") { }

    struct grimrail_depot_skylord_torva_at_freezing_snareAI : AreaTriggerAI
    {
        grimrail_depot_skylord_torva_at_freezing_snareAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }
        uint32 timer = IN_MILLISECONDS;
        ...
        void OnUpdate(uint32 diff) override
        {
            ...
            if (Player* player = at->FindNearestPlayer(2.0f, true))
            {
                if (Unit* caster = at->GetCaster())
                    caster->CastSpell(player, 162065, true);
                at->Remove();
            }
            timer = IN_MILLISECONDS;
        }
    ...
    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new grimrail_depot_skylord_torva_at_freezing_snareAI(areaTrigger);
    }
};
