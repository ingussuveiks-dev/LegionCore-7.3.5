...
class boss_nitrogg_thundertower : public CreatureScript
{
    ...
    struct boss_nitrogg_thundertowerAI : public BossAI
    {
        ...
        std::set<ObjectGuid> m_PushedPlayers;
        bool m_First;
        ...
        void Reset() override
        {
            events.Reset();
            m_TargetGUID.Clear();
            for (ObjectGuid const& guid : m_PushedPlayers)
                if (Player* player = ObjectAccessor::FindPlayer(guid))
                    player->SendMovementForceAura(me->GetGUID(), Position(), Position(), 0.0f, 0, false);
            m_PushedPlayers.clear();
        ...
        void JustReachedHome() override
        {
            ...
            if (!l_TurretsList.empty())
            {
                for (Creature* l_Itr : l_TurretsList)
                {
                    ...
                    l_Itr->Respawn(true);
                    l_Itr->NearTeleportTo(l_Itr->GetHomePosition().GetPositionX(), l_Itr->GetHomePosition().GetPositionY(), l_Itr->GetHomePosition().GetPositionZ(), l_Itr->GetHomePosition().GetOrientation());
                }
            }
        }
        ...
        void MovementInform(uint32 /*p_Type*/, uint32 p_Id) override
        {
            ...
            case eNitroggThundertowerMovments::MovementPointThreeRemoveFlags:
                ...
                me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_IMMUNE_TO_PC );
                break;
            default:
                break;
            }
        ...
        void UpdateAI(uint32 const p_Diff) override
        {
            ...
            if (m_PushbackDiff <= p_Diff)
            {
                ...
                if (!l_ListPlayers.empty())
                {
                    ...
                    for (Player* l_Itr : l_ListPlayers)
                    {
                        ...
                        Creature* trigger = l_Itr->FindNearestCreature(GrimrailDepotCreatures::CreatureNitroggPushBackTrigger, 10.0f);
                        bool shouldPush = trigger && !l_Itr->IsWithinLOSInMap(trigger) &&
                            l_Itr->IsWithinDist(me, 100.0f, true) && l_Itr->IsAlive();
                        bool isPushed = m_PushedPlayers.count(l_Itr->GetGUID()) != 0;

                        if (shouldPush && !isPushed)
                        {
                            l_Itr->SendMovementForceAura(me->GetGUID(), Position(), l_Position, -3.0f, 1, true);
                            m_PushedPlayers.insert(l_Itr->GetGUID());
                        }
                        else if (!shouldPush && isPushed)
                        {
                            l_Itr->SendMovementForceAura(me->GetGUID(), Position(), Position(), 0.0f, 0, false);
                            m_PushedPlayers.erase(l_Itr->GetGUID());
                        }
                    }
                }
            ...
            case eNitroggThundertowerEvents::EventReinforceBoomers:
                for (uint8 l_I = 0; l_I < 3; l_I++)
                {
                    ...
                    {
                        float l_X = l_Creature->GetPositionX() + 15 * std::cos(l_Creature->GetOrientation());
                        float l_Y = l_Creature->GetPositionY() + 15 * std::sin(l_Creature->GetOrientation());
...
class grimrail_depot_nitrogg_thundertower_mob_iron_turret : public CreatureScript
{
    ...
    struct grimrail_depot_nitrogg_thundertower_mob_iron_turretAI : public ScriptedAI
    {
        ...
        {
            events.Reset();
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_REMOVE_CLIENT_CONTROL | UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC);
        }
            ...
            case eGromkarboomerEvents::EventBlackrockMortarBombs:
            {
                ...
                if (Vehicle* l_Vehicle = me->GetVehicleKit())
                {
                    if (Unit* l_Passenger = l_Vehicle->GetPassenger(0))
                    {
                        ...
                        {
                            m_Modifier += 2;
                            float l_X = me->GetPositionX() + (2 + m_Modifier) * std::cos(me->GetOrientation());
                            float l_Y = me->GetPositionY() + (2 + m_Modifier) * std::sin(me->GetOrientation());
...
class grimrail_depot_nitrogg_thundertower_mob_assault_turret : public CreatureScript
{
    ...
    struct grimrail_depot_nitrogg_thundertower_mob_assault_turretAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            events.Reset();
            me->SetReactState(ReactStates::REACT_PASSIVE);
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_REMOVE_CLIENT_CONTROL | UNIT_FLAG_IMMUNE_TO_NPC);
        }
                ...
                if (Unit* l_Target = GetTarget())
                {
                    if (l_Target->IsWithinLOSInMap(l_Caster))
                    {
                        ...
                        if (Creature* l_NearestTrigger = l_Caster->SummonCreature(eSuppressiveFireCreatures::CreatureIronTurretNitroggSuppressiveFire, l_Position, TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 1 * TimeConstants::IN_MILLISECONDS))
                        {
                            ...
                            l_NearestTrigger->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_REMOVE_CLIENT_CONTROL);
                            l_Caster->SendPlaySpellVisualKit(eNitroggThundertowerSpellVisuals::SpellFireSuppressionVisualId, 0);
...
/// Slagblast - 166571
class grimrail_depot_nitrogg_thundertower_at_slagblast : public AreaTriggerScript
{
public:
    grimrail_depot_nitrogg_thundertower_at_slagblast() : AreaTriggerScript("grimrail_depot_nitrogg_thundertower_at_slagblast") { }

    struct grimrail_depot_nitrogg_thundertower_at_slagblastAI : AreaTriggerAI
    {
        grimrail_depot_nitrogg_thundertower_at_slagblastAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }

        std::set<ObjectGuid> targets;
        uint32 timer = 2 * IN_MILLISECONDS;

        void OnCreate() override
        {
            std::list<Creature*> triggers;
            at->GetCreatureListWithEntryInGrid(triggers, 543533, 200.0f);
            triggers.sort([this](Creature const* left, Creature const* right)
            {
                return at->GetExactDist2d(left) < at->GetExactDist2d(right);
            });

            for (Creature* trigger : triggers)
                trigger->CastSpell(trigger, 166571);
        }
        ...
        void OnUpdate(uint32 diff) override
        {
            ...
            at->GetPlayerListInGrid(players, 2.0f);
            for (Player* player : players)
                if (!player->HasAura(166570))
                {
                    player->CastSpell(player, 166570, true);
        ...
        void OnRemove() override
        {
            for (ObjectGuid const& guid : targets)
                if (Unit* target = ObjectAccessor::GetUnit(*at, guid))
                    target->RemoveAura(166570);
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new grimrail_depot_nitrogg_thundertower_at_slagblastAI(areaTrigger);
    }
};
