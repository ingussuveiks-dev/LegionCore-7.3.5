    ...
    if (p_InstanceScript != nullptr)
    {
        ...
        case 1: /// Door 1
            if (p_InstanceScript->GetData(GrimrailDepotData::DataIronWroughtGateOnTrainCount) < 3)
                p_InstanceScript->SetData(GrimrailDepotData::DataIronWroughtGateOnTrainCount, p_InstanceScript->GetData(GrimrailDepotData::DataIronWroughtGateOnTrainCount) + 1);
    ...
    struct grimrail_depot_mob_grimrail_overseerAI : public ScriptedAI
    {
        ...
        void MovementInform(uint32 /*p_Type*/, uint32 p_Id) override
        {
            ...
            case eMovementInformed::MovementInformedDashSecondCut:
            {
                ...
                me->GetMotionMaster()->MoveCharge(l_Position.GetPositionX(), l_Position.GetPositionY(), l_Position.GetPositionZ(), 24.0f, eMovementInformed::MovementInformedDashFinish);
            }
            break;
...
public:
    ...
    bool OnGossipHello(Player * p_Player, Creature * p_Creature) override
    {
        ...
        p_Player->SendSpellScene(GrimrailDepotScenes::SceneBoardToGrimrail, nullptr, true, p_Player);
        ...
        void HandleDummy(SpellEffIndex /*p_EffIndex*/)
        {
            if (Unit* l_Caster = GetCaster())
            {
                if (Unit* l_Target = GetExplTargetUnit())
                    l_Caster->GetMotionMaster()->MoveCharge(l_Target->GetPositionX(), l_Target->GetPositionY(), l_Target->GetPositionZ(), 24.0f, eMovementInformed::MovementInformedDashSecondCut, false, l_Target);
...
/// Flametongue - 176033
class grimrail_depot_at_flametongue : public AreaTriggerScript
{
public:
    grimrail_depot_at_flametongue() : AreaTriggerScript("grimrail_depot_at_flametongue") { }

    struct grimrail_depot_at_flametongueAI : AreaTriggerAI
    {
        grimrail_depot_at_flametongueAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }
        ...
        void OnUpdate(uint32 diff) override
        {
            ...
            for (Player* player : players)
                if (!player->HasAura(176033))
                {
                    player->CastSpell(player, 176033, true);
        ...
        void OnRemove() override
        {
            for (ObjectGuid const& guid : targets)
                if (Unit* target = ObjectAccessor::GetUnit(*at, guid))
                    target->RemoveAura(176033);
        }
    };

    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new grimrail_depot_at_flametongueAI(areaTrigger);
    }
};

/// Healing Rain - 166387
class grimrail_depot_at_healing_rain : public AreaTriggerScript
{
public:
    grimrail_depot_at_healing_rain() : AreaTriggerScript("grimrail_depot_at_healing_rain") { }

    struct grimrail_depot_at_healing_rainAI : AreaTriggerAI
    {
        grimrail_depot_at_healing_rainAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }
        uint32 timer = 3 * IN_MILLISECONDS;
        ...
        void OnUpdate(uint32 diff) override
        {
            ...
            if (Unit* caster = at->GetCaster())
            {
                ...
                at->GetCreatureListInGrid(creatures, 3.0f);
                for (Creature* creature : creatures)
                    if (caster->IsFriendlyTo(creature))
                        caster->CastSpell(creature, 142924, true);
            }
    ...
    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new grimrail_depot_at_healing_rainAI(areaTrigger);
    }
};

/// Thunder Zone - 166341
class grimrail_depot_at_thunder_zone : public AreaTriggerScript
{
public:
    grimrail_depot_at_thunder_zone() : AreaTriggerScript("grimrail_depot_at_thunder_zone") { }

    struct grimrail_depot_at_thunder_zoneAI : AreaTriggerAI
    {
        grimrail_depot_at_thunder_zoneAI(AreaTrigger* areaTrigger) : AreaTriggerAI(areaTrigger) { }
        uint32 timer = 3 * IN_MILLISECONDS;
        ...
        void OnUpdate(uint32 diff) override
        {
            ...
            if (Unit* caster = at->GetCaster())
                for (Unit* target : targets)
                    caster->CastSpell(target, 166340, true);
            timer = 3 * IN_MILLISECONDS;
        }
    };
    ...
    AreaTriggerAI* GetAI(AreaTrigger* areaTrigger) const override
    {
        return new grimrail_depot_at_thunder_zoneAI(areaTrigger);
    }
};
...
void AddSC_grimrail_depot()
{
    ...
    new grimrail_depot_iron_star();                    /// 81293
    new grimrail_depot_mob_board_to_grimrail();        /// 86013
