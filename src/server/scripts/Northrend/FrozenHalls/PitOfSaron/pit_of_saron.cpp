...
public:
    ...
    struct npc_jaina_POSintroAI: public ScriptedAI
    {
        ...
        {
            for (ObjectGuid const& skeletonGuid : m_uiSkeletGuid)
                if (Creature* skeleton = me->GetMap()->GetCreature(skeletonGuid))
                    me->CastSpell(skeleton, SPELL_NECROMANTIC_POWER, true);
        }
