            ...
            void DoAction(int32 const action) override
            {
                switch (action)
                {
                    ...
                    case ACTION_MONITOR_CORPOREALITY:
                    {
                        ...
                        // The original implementation used a server-only spell_dbc row that does not exist in Legion.
                        for (float yOffset : { 20.0f, -20.0f })
                        {
                            Position portalPosition = me->GetPosition();
                            portalPosition.m_positionY += yOffset;
                            me->SummonGameObject(GO_HALION_PORTAL_EXIT, portalPosition, me->GetOrientation(),
