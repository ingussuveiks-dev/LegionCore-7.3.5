param(
    ...
    [string]$OutputPath = "$PSScriptRoot/../../build-extractors/extracted/legion-collectible-catalog-generated.sql"
)
...
$sql.Add('-- 7.3.5 Store cards use CreatureDisplayInfoID as DisplayId and scene 10 as VisualId.')
...
$sql.Add('SELECT c.`ProductID`,c.`ModelID`,10,0,c.`Name`,IF(c.`GroupID`=2,''Mount'',''Battle pet''),')
...
$sql.Add('SELECT c.`ProductID`,c.`ModelID`,10,c.`Name` FROM `_battlepay_collectibles_317` c WHERE c.`ProductID` >= 154;')
$sql.Add('')
